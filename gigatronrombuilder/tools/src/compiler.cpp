...
namespace Compiler
{
    ...
    enum VarResult {VarError=-1, VarNotFound, VarCreated, VarUpdated, VarExists, VarIsDifferentType};
    enum StrResult {StrError=-1, StrNotFound, StrCreated};
    enum FloatSize {Float16=2, Float32=4};
    enum LabelResult {LabelError=-1, LabelSkipped, LabelNotFound, LabelFound};
    ...
    uint16_t _blitsAddrLutAddress  = 0x0000;
    uint16_t _blitStripeChunks     = 15;
    uint16_t _blitStripeMinAddress = USER_CODE_START;
    uint16_t _blitStripeMaxAddress = RUN_TIME_START;

    Memory::FitType _blitStripeFitType = Memory::FitDescending;
    ...
    std::set<std::string> _pragmaDefines;
    ...
    std::map<std::string, uint16_t> _openDevices =
    {
        {"NUL_DEV",    uint16_t(0x0000)},
        {"ARD_SDCARD", uint16_t(0x0100)}, {"ARD_FLASH", uint16_t(0x0101)}, {"ARD_HOST", uint16_t(0x0102)},
        {"EXP_SDCARD", uint16_t(0x0200)}
    };

    std::map<int, DefDataBlit> _defDataBlits;
    BlitsAddrLut _blitsAddrLut;
    ...
    std::map<int, DefDataPattern> _defDataPatterns;
    std::map<int, PatternAddrLut> _patternAddrLuts;
    PatternsAddrLut _patternsAddrLut;
    ...
    uint16_t getBlitsAddrLutAddress(void) {return _blitsAddrLutAddress;}
    uint16_t getBlitStripeChunks(void) {return _blitStripeChunks;}
    uint16_t getBlitStripeMinAddress(void) {return _blitStripeMinAddress;}
    uint16_t getBlitStripeMaxAddress(void) {return _blitStripeMaxAddress;}
    Memory::FitType getBlitStripeFitType(void) {return _blitStripeFitType;}
    ...
    uint8_t getAudioBitMask(void) {return _audioBitMask;}
    ...
    int getNextVasmLabelIndex(void) {return _nextVasmLabelIndex;}
    void setNextVasmLabelIndex(int nextVasmLabelIndex) {_nextVasmLabelIndex = nextVasmLabelIndex;}

    void setAssignVar(bool assignVar) {_assignVar = assignVar;}
    void setCodeIsAsm(bool codeIsAsm) {_codeIsAsm = codeIsAsm;}
    void setAudioBitMask(uint8_t audioBitMask) {_audioBitMask = audioBitMask;}
    ...
    void setBlitsAddrLutAddress(uint16_t blitsAddrLutAddress) {_blitsAddrLutAddress = blitsAddrLutAddress;}
    void setBlitStripeChunks(uint16_t blitStripeChunks) {_blitStripeChunks = blitStripeChunks;}
    void setBlitStripeMinAddress(uint16_t blitStripeMinAddress) {_blitStripeMinAddress = blitStripeMinAddress;}
    void setBlitStripeMaxAddress(uint16_t blitStripeMaxAddress) {_blitStripeMaxAddress = blitStripeMaxAddress;}
    void setBlitStripeFitType(Memory::FitType blitStripeFitType) {_blitStripeFitType = blitStripeFitType;}
    ...
    int getNextFlowControlUniqueId(void) {return _flowControlUniqueId++;}
    ...
    std::set<std::string>& getPragmaDefines(void) {return _pragmaDefines;}
    ...
    std::map<std::string, uint16_t>& getOpenDevices(void) {return _openDevices;}

    std::map<int, DefDataBlit>& getDefDataBlits(void) {return _defDataBlits;}
    BlitsAddrLut& getBlitsAddrLut(void) {return _blitsAddrLut;}
    ...
    std::map<int, DefDataPattern>& getDefDataPatterns(void) {return _defDataPatterns;}
    std::map<int, PatternAddrLut>& getPatternAddrLuts(void) {return _patternAddrLuts;}
    PatternsAddrLut& getPatternsAddrLut(void) {return _patternsAddrLut;}
    ...
    bool isROMvX0(void)
    {
        if((_codeRomType >= Cpu::ROMv6  &&  _codeRomType < Cpu::ROMvX0)  ||  (_codeRomType >= Cpu::ROMvX0  &&  _codeRomType < Cpu::SDCARD))
        {
            return true;
        }
    ...
    void adjustLabelAddresses(uint16_t address, int offset)
    {
        ...
        for(int i=0; i<int(_labels.size()); i++)
        {
            ...
            {
                _labels[i]._address += int16_t(offset);
            }
        }
    }
    ...
    {
        for(int i=codeLineIndex; i<int(getCodeLines().size()); i++)
        {
            for(int j=0; j<int(getCodeLines()[i]._vasm.size()); j++)
            {
                // Don't adjust page jump's
                if(!getCodeLines()[i]._vasm[j]._pageJump  &&  getCodeLines()[i]._vasm[j]._address >= address)
                {
                    getCodeLines()[i]._vasm[j]._address += int16_t(offset);
                }
            }
        }
    }

    bool checkIntVar(const CodeLine& codeLine, int codeLineIndex)
    {
        if(getCodeRomType() < Cpu::ROMv5a)
        {
            if(_userVarsAddr > USER_VAR_END)
            {
                _userVarsAddr = _userVarStart;
                Cpu::reportError(Cpu::WarnError, stderr, "Compiler::checkIntVar() : '%s:%d' : warning, you have used the maximum number of page zero variables : %s\n",
        ...
        // ROMv5a or higher
        if(_userVarsAddr >= LOCAL_VAR_START)
        {
            _userVarsAddr = _userVarStart;
            Cpu::reportError(Cpu::WarnError, stderr, "Compiler::checkIntVar() : '%s:%d' : warning, you have used the maximum number of page zero variables : %s\n",
                                                      codeLine._moduleName.c_str(), codeLineIndex, codeLine._text.c_str());
        ...
        // Zero page space from CONVERT_CC_OPS to REAL_TIME_PROC-1, (Compiler.h:30), is not being used for romType >= ROMv5a, so use it for extra global/fast int16 var's
        // ROMvX0 requires CONVERT_CC_OPS to CONVERT_CC_OPS + 15 to be free for new instructions
        else if(_userVarsAddr == USER_VAR_END + 1)
        {
            _userVarsAddr = (_codeRomType >= Cpu::ROMvX0  &&  _codeRomType < Cpu::SDCARD) ? CONVERT_CC_OPS + 16 : CONVERT_CC_OPS;
        }

        return true;
    }

    // ROMv5a vertical blank interrupt uses 0x30-0x35 for context save/restore
    // ROMvX0 vertical blank interrupt uses 0x30-0x37 for context save/restore
    bool moveVblankVars(const Compiler::CodeLine& codeLine, int codeLineIndex)
    {
        if(_codeRomType < Cpu::ROMv5a) return false;

        uint16_t sizeVblankVars = (_codeRomType < Cpu::ROMvX0  ||  _codeRomType >= Cpu::SDCARD) ? Int16*3 : Int16*4;

        if(_userVarsAddr < USER_VAR_START + sizeVblankVars)
        {
            _userVarStart = USER_VAR_START + sizeVblankVars;
            _userVarsAddr = _userVarStart;
        }

        for(int i=0; i<int(_integerVars.size()); i++)
        {
            if(_integerVars[i]._address == USER_VAR_START  ||  _integerVars[i]._address == USER_VAR_START + Int16  ||  _integerVars[i]._address == USER_VAR_START + Int16*2  ||
               (sizeVblankVars == Int16*4  &&  _integerVars[i]._address == USER_VAR_START + Int16*3))
            {
                _integerVars[i]._address = _userVarsAddr;
                _userVarsAddr += Int16;
                if(!checkIntVar(codeLine, codeLineIndex)) return false;
            }
        }
    ...
    bool readInputFile(std::ifstream& infile, const std::string& filename, std::vector<Input>& input, int& numLines)
    {
        if(!infile.is_open())
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::readInputFile() : Failed to open file '%s'\n", filename.c_str());
            return false;
        }
        ...
        while(!infile.eof())
        {
            ...
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::readInputFile() : '%s:%d' : compound statement must span multiple lines\n", filename.c_str(), numLines+1);
                return false;
            }
            ...
            if(Expression::findNonStringBracket(lineToken, '{') != lineToken.end())
            {
                do
                {
                    ...
                    if(infile.eof())
                    {
                        Cpu::reportError(Cpu::ComError, stderr, "Compiler::readInputFile() : '%s:%d' : missing '}'\n", filename.c_str(), numLines+1);
                        return false;
                    }
            ...
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::readInputFile() : '%s:%d' : bad line '%s'\n", filename.c_str(), numLines+1, lineToken.c_str());
                return false;
            }
    ...
    bool writeOutputFile(std::ofstream& outfile, const std::string& filename)
    {
        if(!outfile.is_open())
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::writeOutputFile() : failed to open '%s'\n", filename.c_str());
            return false;
        }
        ...
        for(int i=0; i<int(_output.size()); i++)
        {
            ...
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::writeOutputFile() : write error in '%s'\n", filename.c_str());
                return false;
            }
        }
    ...
    int findLabelName(const std::string& labelName)
    {
        for(int i=0; i<int(_labels.size()); i++)
        {
    ...
    int findLabelAddr(uint16_t address)
    {
        for(int i=0; i<int(_labels.size()); i++)
        {
    ...
    int findLabel(const std::string& labelName, uint16_t address)
    {
        int labelIndex = findLabelName(labelName);
        if(labelIndex == -1)
        {
            labelIndex = findLabelAddr(address);
        }
    ...
    {
        Compiler::VarType varTypeC = ConstInt16;
        switch(varTypeE)
        {
            case Expression::IntVar16:  varTypeC = VarInt16;  break;
            case Expression::Arr1Var16: varTypeC = Var1Arr16; break;
            case Expression::Arr2Var16: varTypeC = Var2Arr16; break;
            case Expression::Arr3Var16: varTypeC = Var3Arr16; break;
            case Expression::Arr1Var8:  varTypeC = Var1Arr8;  break;
            case Expression::Arr2Var8:  varTypeC = Var2Arr8;  break;
            case Expression::Arr3Var8:  varTypeC = Var3Arr8;  break;
            case Expression::StrVar:    varTypeC = VarStr;    break;
            case Expression::Str2Var:   varTypeC = VarStr2;   break;
    ...
    {
        Expression::VarType varTypeE = Expression::Number;
        switch(varTypeC)
        {
            case VarInt16:  varTypeE = Expression::IntVar16;  break;
            case Var1Arr16: varTypeE = Expression::Arr1Var16; break;
            case Var2Arr16: varTypeE = Expression::Arr2Var16; break;
            case Var3Arr16: varTypeE = Expression::Arr3Var16; break;
            case Var1Arr8:  varTypeE = Expression::Arr1Var8;  break;
            case Var2Arr8:  varTypeE = Expression::Arr2Var8;  break;
            case Var3Arr8:  varTypeE = Expression::Arr3Var8;  break;
            case VarStr:    varTypeE = Expression::StrVar;    break;
            case VarStr2:   varTypeE = Expression::Str2Var;   break;
    ...
    int createLabel(uint16_t address, const std::string& name, int codeLineIndex, Label& label, bool skipAddr, bool numeric, bool addUnderscore, bool pageJump, bool gosub, bool noPush)
    {
        int labelIndex = -1;
        if(skipAddr)
        {
            labelIndex = findLabelName(name);
        }
        else
        {
            labelIndex = findLabel(name, address);
        ...
        label = {address, n, o, codeLineIndex, numeric, pageJump, gosub, noPush};
        Expression::stripWhitespace(label._name);
        // Sorted label vector during creation, (requires manual sorting whenever vector is updated)
        //auto it = std::lower_bound(_labels.begin(), _labels.end(), address, [](const Compiler::Label& lab, uint16_t addr)
        //                                                                      {return lab._address < addr;});
    ...
    bool createIntVar(const std::string& varName, int16_t data, int16_t init, CodeLine& codeLine, int codeLineIndex, bool containsVars, int& varIndex)
    {
        ...
        _userVarsAddr += Int16;
        return checkIntVar(codeLine, codeLineIndex);
    }
    ...
    void createProcIntVar(const std::string& varName, int16_t data, int16_t init, CodeLine& codeLine, int codeLineIndex, bool containsVars, uint16_t address, int& varIndex)
    {
        ...
        for(int i=0; i<int(_integerVars.size()); i++)
        {
            if(_integerVars[i]._name == varName)
            {
                varIndex = -1;
                return;
            }
    ...
    VarResult createCodeVar(CodeLine& codeLine, int codeLineIndex, int& varIndex)
    {
        ...
        if(findConst(constName) >= 0)
        {
            Cpu::reportError(Cpu::ComError, stderr, "Keywords::createCodeVar() : '%s:%d' : const '%s' already exists : %s\n", codeLine._moduleName.c_str(), codeLineIndex, constName.c_str(), codeLine._text.c_str());
            return VarIsDifferentType;
        }
        ...
        if(varIndex != -1)
        {
            ...
            codeLine._varIndex = varIndex;
            codeLine._varType = _integerVars[varIndex]._varType;

            return VarExists;
        }

        if(!createIntVar(varName, 0, 0, codeLine, codeLineIndex, false, varIndex)) return VarError;
    ...
    bool createAsmLine(const std::string& code)
    {
        ...
        if(tokens.size() < 1)
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::createAsmLine() : '%s:%d' : vASM syntax error, missing opcode : %s\n", codeLine._moduleName.c_str(), _currentCodeLineIndex, codeLine._text.c_str());
            return false;
        }
        ...
        if(vasmSize == 0  &&  opcodeStr != "_BREAKPOINT_")
        {
            ...
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::createAsmLine() : '%s:%d' : vASM syntax error, undefined opcode : %s\n", codeLine._moduleName.c_str(), _currentCodeLineIndex, codeLine._text.c_str());
            return false;
        }
    ...
    bool createCodeLine(const std::string& code, int codeLineStart, int labelIndex, int varIndex, Expression::Int16Byte int16Byte, bool vars, CodeLine& codeLine, const std::string& moduleName)
    {
        ...
        codeLine = {text, codeText, tokens, offsets, vasm, expression, onGotoGosubLut, strConcatLut, inputLut, 0, labelIndex, varIndex, VarInt16, int16Byte, vars, false, false, true, moduleName};
    ...
    bool initialiseMacros(void)
    {
        ...
        {
            filename = "/macros_ROMvX0.i";
        }
        else if(_codeRomType >= Cpu::ROMv5a)
        {
            filename = "/macros_ROMv5a.i";
        }
        else
        {
            filename = "/macros.i";
        ...
        if(!infile.is_open())
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::initialiseMacros() : failed to open file '%s'\n", filename.c_str());
            return false;
        }
        ...
        if(foundMacro)
        {
            ...
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::initialiseMacros() : '%s:%d' : %%MACRO '%s' is missing a %%ENDM\n", filename.c_str(), macroIndex, macroName.c_str());
            return false;
        }
    ...
    bool initialiseCode(void)
    {
        ...
        Label label;
        createLabel(_vasmPC, "entryPoint_", 0, label, true);
        ...
        if(_codeRomType > Cpu::ROMv1  &&  _enableRomCheck)
        {
            // Don't use any new instructions, (>ROMv4) until RomCheck is complete!
            emitVcpuAsm("LDI", Expression::byteToHexString(uint8_t(_codeRomType)), false, 0);
            emitVcpuAsm("STW", "romUser", false, 0);
            emitVcpuAsm("%RomCheck", "", false, 0);
        }

        // Initialise
        (_enableInitialise) ? emitVcpuAsm("%InitialiseOn", "", false, 0) : emitVcpuAsm("%InitialiseOff", "", false, 0);

        // ROMvX0 audio bit mask
        if(_codeRomType >= Cpu::ROMvX0  &&  _codeRomType < Cpu::SDCARD)
        {
            if(getAudioBitMask() != 0x0f)
            {
                emitVcpuAsm("MOVQB", "0x08, " + Expression::byteToHexString(getAudioBitMask()), false, 0);
            }
        }
    ...
    int createVcpuAsm(const std::string& opcodeStr, const std::string& operandStr, int codeLineIdx, std::string& line, Assembler::VACType& vAcType)
    {
        ...
        if(opcode.size())
        {
            ...
            if(_macroIndexEntries.find(opcode) != _macroIndexEntries.end())
            {
                ...
                if(!foundPercent)
                {
                    CodeLine codeLine = _codeLines[codeLineIdx];
                    Cpu::reportError(Cpu::WarnError, stderr, "Compiler::createVcpuAsm() : '%s:%d' : internal warning, Macro '%s' is missing '%%' in the compiler's source code : %s\n", codeLine._moduleName.c_str(), codeLineIdx,
                                                                                                                                                                                        opcode.c_str(), codeLine._text.c_str());
                }
            }
        ...
        //Cpu::reportError(Cpu::NoError, stderr, "%s  %d %04x\n", opcode.c_str(), vasmSize, _vasmPC);

        std::string operand = std::string(operandStr);
        line = Expression::createPaddedString(opcode, OPCODE_TRUNC_SIZE, ' ') + operand;
    ...
    {
        if(_codeLines[codeLineIdx]._vasm.size() == 0) return false;

        _codeLines[codeLineIdx]._vasm.back()._lazyErase = true;

        return true;
    }

    std::pair<int, int> emitVcpuAsm(const std::string& opcodeStr, const std::string& operandStr, bool nextTempVar, int codeLineIdx, const std::string& labelName, bool pageJump)
    {
        ...
        Label label;
        int labelIndex = -1;
        if(labelName.size()  &&  _nextVasmLabelIndex == -1)
        {
            labelIndex = createLabel(_vasmPC, labelName, codeLineIdx, label);
        }
        ...
        static int breakPointSlot = -1;
        if(labelName.size()  &&  Expression::strUpper(opcodeStr) == "_BREAKPOINT_")
        {
    ...
    int16_t emitVcpuAsmForLiteral(Expression::Numeric& numeric, bool useHex)
    {
        ...
        {
            emitVcpuAsm("LDWI", "_" + numeric._name, false);
        }
        else if(value >= 0  &&  value <= 255)
        {
            std::string literalStr = (useHex) ? Expression::byteToHexString(uint8_t(value)) : std::to_string(value);
            emitVcpuAsm("LDI", literalStr, false);
        }
        else
        {
            std::string literalStr = (useHex) ? Expression::wordToHexString(value) : std::to_string(value);
            emitVcpuAsm("LDWI", literalStr, false);
        }
    ...
    int16_t emitVcpuAsmForLiteral(const std::string& opcode, Expression::Numeric& numeric, bool useHex)
    {
        ...
        if(numeric._lazyUpdate)
        {
            emitVcpuAsm(opcode, "_" + numeric._name, false);
        }
        else if(value >= 0  &&  value <= 255)
        {
            std::string literalStr = (useHex) ? Expression::byteToHexString(uint8_t(value)) : std::to_string(value);
            emitVcpuAsm(opcode, literalStr, false);
        }
        else
        {
            std::string literalStr = (useHex) ? Expression::wordToHexString(value) : std::to_string(value);
    ...
    bool erasePragmaDefine(const std::string& defineName)
    {
        return (_pragmaDefines.erase(defineName) > 0);
    }

    void pragmaPreProcessingCmd(const std::string& cmdStr, int lineIndex)
    {
        _pragmaDefines.insert(cmdStr);

        std::vector<std::string> tokens = Expression::tokenise(cmdStr, ' ', false);
        Assembler::createDefine("", tokens, lineIndex);
    }

    void emitVcpuPreProcessingCmd(const std::string& cmdStr)
    {
        _codeLines[0]._vasm.push_back({_userCodeStart, cmdStr, "", cmdStr, false, false, -1, 0});
    }

    void createVcpuAsmLabel(int codeLineIdxBra, int vcpuAsmBra, const std::string& labelName)
    {
        std::string opcode = _codeLines[codeLineIdxBra]._vasm[vcpuAsmBra]._opcode;
        _codeLines[codeLineIdxBra]._vasm[vcpuAsmBra]._code = Expression::createPaddedString(opcode, OPCODE_TRUNC_SIZE, ' ') + labelName;
        _codeLines[codeLineIdxBra]._vasm[vcpuAsmBra]._operand = labelName;

        Label label;
        int labelIndex = -1;
        if(labelName.size())
        {
            labelIndex = createLabel(_vasmPC, labelName, codeLineIdxBra, label);
        }
    }
    ...
    bool writeArray2d(CodeLine& codeLine, int codeLineIndex, size_t lbra, size_t rbra, int intSize, uint16_t arrayPtr)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::writeArray2d() : '%s:%d' : number of dimensions must be equal to 2, found %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex,
        ...
        if(_codeRomType >= Cpu::ROMvX0  &&  _codeRomType < Cpu::SDCARD)
        {
            switch(intSize)
            {
                case Int8:
                {
                    //emitVcpuAsm("CALLI", "convert8Arr2d", false, codeLineIndex);
                    emitVcpuAsm("ADDW",  "memIndex0", false, codeLineIndex);
                    emitVcpuAsm("ADDW",  "memIndex0", false, codeLineIndex);
                    emitVcpuAsm("DEEK",  "",          false, codeLineIndex);
                    emitVcpuAsm("ADDW",  "memIndex1", false, codeLineIndex);
                ...
                case Int16:
                {
                    emitVcpuAsm("CALLI", "convert16Arr2d", false, codeLineIndex);
                    switch(codeLine._int16Byte)
                    {
                        case Expression::Int16Low:  emitVcpuAsm("POKEA", "memValue", false, codeLineIndex);                                                         break;
                        case Expression::Int16High: emitVcpuAsm("ADDI",  "1",        false, codeLineIndex); emitVcpuAsm("POKEA", "memValue", false, codeLineIndex); break;
                        case Expression::Int16Both: emitVcpuAsm("DOKEA", "memValue", false, codeLineIndex);                                                         break;
    ...
    bool writeArray3d(CodeLine& codeLine, int codeLineIndex, size_t lbra, size_t rbra, int intSize, uint16_t arrayPtr)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::writeArray3d() : '%s:%d' : number of dimensions must be equal to 3, found %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex,
                                                                                                                                                     int(indexTokens.size()), codeLine._text.c_str());
            return false;
        }
        for(int i=0; i<int(indexTokens.size()); i++)
    ...
    // TODO: fix for multi-dimensional arrays and fix for arrays that cross page boundaries
    bool assignArray(CodeLine& codeLine, int codeLineIndex, int varIndex, size_t equals)
    {
        if(equals + 1 >= int(codeLine._code.size())) return false;
        std::string arrayToken = codeLine._code.substr(equals + 1);
        Expression::stripWhitespace(arrayToken);

        Expression::Numeric numeric;
        if(!Expression::parse(arrayToken, codeLineIndex, numeric)) return false;
        if(_integerVars[varIndex]._varType != convertVarType(numeric._varType)) return false;
        if(_integerVars[varIndex]._varType != Var1Arr8  &&  _integerVars[varIndex]._varType != Var1Arr16) return false;

        uint16_t length = std::min(_integerVars[varIndex]._arrSizes[2], _integerVars[numeric._index]._arrSizes[2]);
        if(_codeRomType >= Cpu::ROMvX0  &&  _codeRomType < Cpu::SDCARD)
        {
            emitVcpuAsm("LDWI", Expression::wordToHexString(_integerVars[numeric._index]._arrAddrs[0][0]), false, codeLineIndex);
            emitVcpuAsm("STW", "giga_sysArg0", false, codeLineIndex);
            emitVcpuAsm("LDWI", Expression::wordToHexString(_integerVars[varIndex]._arrAddrs[0][0]), false, codeLineIndex);
            emitVcpuAsm("STW", "giga_sysArg2", false, codeLineIndex);
            emitVcpuAsm("MOVQB", "cpyCount, " + std::to_string(length), false, codeLineIndex);
        }
        else
        {
            emitVcpuAsm("LDWI", Expression::wordToHexString(_integerVars[numeric._index]._arrAddrs[0][0]), false, codeLineIndex);
            emitVcpuAsm("STW", "cpySrcAddr", false, codeLineIndex);
            emitVcpuAsm("LDWI", Expression::wordToHexString(_integerVars[varIndex]._arrAddrs[0][0]), false, codeLineIndex);
            emitVcpuAsm("STW", "cpyDstAddr", false, codeLineIndex);
            emitVcpuAsm("LDI", std::to_string(length), false, codeLineIndex);
            emitVcpuAsm("ST", "cpyCount", false, codeLineIndex);
        }

        switch(_integerVars[varIndex]._varType)
        {
            case Var1Arr8:  emitVcpuAsm("%CopyBytes", "", false, codeLineIndex); break;
            case Var1Arr16: emitVcpuAsm("%CopyWords", "", false, codeLineIndex); break;
    ...
    bool writeArrayVar(CodeLine& codeLine, int codeLineIndex, int varIndex)
    {
        ...
        size_t lbra, rbra;
        std::string outArray = codeLine._code.substr(0, equals);
        if(!Expression::findMatchingBrackets(outArray, 0, lbra, rbra))
        {
            // TODO: fix for multi-dimensional arrays and fix for arrays that cross page boundaries
            return assignArray(codeLine, codeLineIndex, varIndex, equals);
        }
        ...
        switch(varType)
        {
            ...
            case VarType::Var1Arr16: result = writeArray1d(codeLine, codeLineIndex, lbra, rbra, intSize, arrayPtr); break;

            case VarType::Var2Arr8:
            case VarType::Var2Arr16: result = writeArray2d(codeLine, codeLineIndex, lbra, rbra, intSize, arrayPtr); break;

            case VarType::Var3Arr8:
            case VarType::Var3Arr16: result = writeArray3d(codeLine, codeLineIndex, lbra, rbra, intSize, arrayPtr); break;
    ...
    bool emitVcpuAsmUserVar(const std::string& opcodeStr, Expression::Numeric& numeric, bool nextTempVar)
    {
        ...
        if(varIndex == -1)
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::emitVcpuAsmUserVar() : couldn't find variable name '%s'\n", varName.c_str());
            return false;
        }
    ...
    uint32_t parseExpression(int codeLineIndex, std::string& expression, Expression::Numeric& numeric)
    {
        ...
        {
            emitVcpuAsmForLiteral(numeric, false);
        }
    ...
    bool checkForGosubLabel(const std::string& moduleName, const std::string& code, int lineNumber)
    {
        ...
        for(int i=0; i<int(tokens.size()); i++)
        {
            if(Expression::strToUpper(tokens[i]) == "GOSUB")
            {
                ...
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForGosubLabel() : '%s:%d' : missing label after GOSUB in '%s'\n", moduleName.c_str(), lineNumber + 1, code.c_str());
                    return false;
                }
    ...
    LabelModifierType getLabelModifiers(const std::string& labelName, std::string& code, size_t colon)
    {
        // Nothing after the colon
        if(labelName.size() <= colon + 1) return LabelModNone;

        // Extract modifier
        static std::map<std::string, LabelModifierType> labelModifiersMap = {{"[NO_PUSH]", LabelModNoPush}};
        std::string modifier = labelName.substr(colon + 1);
        Expression::strToUpper(modifier);

        if(labelModifiersMap.find(modifier) == labelModifiersMap.end()) return LabelModError;

        // Remove label modifier from code
        code.erase(colon + 1, modifier.size());

        return labelModifiersMap[modifier];
    }
    ...
    LabelResult checkForLabel(const std::string& moduleName, std::string& code, int lineNumber)
    {
        ...
        if(code.size() > 1  &&  isdigit((unsigned char)code[0]))
        {
            ...
            {
                //Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForLabel() : '%s:%d' : line number cannot exist on its own in '%s'\n", moduleName.c_str(), lineNumber + 1, code.c_str());
                return LabelSkipped;
            }

            if(code[0] == '0')
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForLabel() : '%s:%d' : line number cannot be zero or start with zero in '%s'\n", moduleName.c_str(), lineNumber + 1, code.c_str());
                return LabelError;
            }
            ...
            if(colon != std::string::npos)
            {
                ...
                switch(getLabelModifiers(labelName, code, colon))
                {
                    case LabelModNone:                       break;
                    case LabelModNoPush: noPushGosub = true; break;

                    case LabelModError:
                    {
                        Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForLabel() : '%s:%d' : syntax error in label modifier\n", moduleName.c_str(), lineNumber + 1);
                        return LabelError;
                    }
                    break;
                ...
                // Erase colon and everything after it
                labelName.erase(colon);
            }
            ...
            int labelIndex = createLabel(_vasmPC, labelName, int(_codeLines.size()), label, true, numeric, true, false, foundGosub, noPushGosub);
            if(!createCodeLine(code, int(space + 1), labelIndex, -1, Expression::Int16Both, false, codeLine, moduleName))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForLabel() : '%s:%d' : createCodeLine() failed in '%s'\n", moduleName.c_str(), lineNumber + 1, code.c_str());
                return LabelError;
            }
        ...
        {
            bool noPushGosub = false;
            LabelModifierType labelModifierType = getLabelModifiers(code, code, colon);
            switch(labelModifierType)
            {
                case LabelModNone:                       break;
                case LabelModNoPush: noPushGosub = true; break;
            ...
            if(Expression::isLabNameValid(labelName))
            {
                ...
                int labelIndex = createLabel(_vasmPC, labelName, int(_codeLines.size()), label, true, false, true, false, foundGosub, noPushGosub);
                ...
                if(code.size() > colon + 1)
                {
                    ...
                    if(labelCode.size() > 2)
                    {
                        if(!createCodeLine(code, int(colon  + 1), labelIndex, -1, Expression::Int16Both, false, codeLine, moduleName))
                        {
                            Cpu::reportError(Cpu::ComError, stderr, "Compiler::checkForLabel() : '%s:%d' : createCodeLine() failed in '%s'\n", moduleName.c_str(), lineNumber + 1, code.c_str());
                            return LabelError;
                        }
                ...
                if(!validCode)
                {
                    _nonNumericLabelIndex = labelIndex;
    ...
    {
        _enableOptimiser = enable;
    }

    void enableRomCheck(bool enable)
    {
        _enableRomCheck = enable;
    }

    void enableInitialise(bool enable)
    {
        _enableInitialise = enable;
    }
    ...
    bool loadModule(const std::string& moduleName, int codeLineIndex, std::vector<Input>& input, int& numLines)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::loadModule() : %s is not a valid module string, use 'MODULE \"<module name>\"'\n", moduleName.c_str());
            return false;
        }
        ...
        if(!infile.is_open())
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::loadModule() : failed to open module %s\n", moduleName.c_str());
            return false;
        }

        //Cpu::reportError(Cpu::NoError, stderr, "Keywords::MODULE() : module path = %s\n", modulePath.c_str());
        ...
        //Cpu::reportError(Cpu::NoError, stderr, "Keywords::MODULE() : module name = %s\n : %d", moduleFile.c_str(), int(Compiler::getCodeLines().size()));

        for(int i=0; i<int(_moduleLines.size()); i++)
        {
            if(moduleFile == _moduleLines[i]._name)
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::loadModule() : module %s already loaded\n", moduleName.c_str());
                return false;
            }
        }
        ...
        if(!readInputFile(infile, modulePath, inp, inpLines)) return false;
    ...
    bool parseModules(std::vector<Input>& input, int& numLines)
    {
        ...
        for(int j=0; j<numLines; j++)
        {
            ...
            if(tokens[0] == "MODULE")
            {
                ...
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::parseModules() : '%s:%d' : syntax error, use 'MODULE \"<module name>\"'\n", input[j]._text.c_str(), j);
                    return false;
                }
    ...
    int getOrCreateString(CodeLine& codeLine, int codeLineIndex, const std::string& str, std::string& name, uint16_t& address, uint8_t maxSize, bool constString, VarType varType)
    {
        ...
        if(constString)
        {
            ...
            else
            {
                ...
                if(!Memory::getFreePageRAM(Memory::FitDescending, strLength + 2, USER_CODE_START, _stringsStart, address))
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::getOrCreateString() : '%s:%d' : not enough RAM for string %s='%s' of size %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex, name.c_str(),
                                                                                                                                                             str.c_str(), strLength + 2, codeLine._text.c_str());
                    return -1;
                }
                ...
                StringVar stringVar = {uint8_t(strLength), uint8_t(strLength), address, str, name, "_" + Expression::createPaddedString(name, LABEL_TRUNC_SIZE - 1, ' '), varType, -1, true, arrInits, arrAddrs};
        ...
        {
            // Allocate RAM for string + length and delimiter bytes
            if(!Memory::getFreePageRAM(Memory::FitDescending, maxSize + 2, USER_CODE_START, _stringsStart, address))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::getOrCreateString() : '%s:%d' : not enough RAM for string %s='%s' of size %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex, name.c_str(),
                                                                                                                                                         str.c_str(), maxSize + 2, codeLine._text.c_str());
                return -1;
            }

            std::vector<uint16_t> arrAddrs;
            std::vector<std::string> arrInits;
            StringVar stringVar = {uint8_t(strLength), maxSize, address, str, name, "_" + Expression::createPaddedString(name, LABEL_TRUNC_SIZE - 1, ' '), varType, -1, false, arrInits, arrAddrs};
            _stringVars.push_back(stringVar);
            index = int(_stringVars.size()) - 1;
        }
    ...
    int createStringArray(CodeLine& codeLine, int codeLineIndex, const std::string& name, uint8_t size, bool isInit, std::vector<std::string>& arrInits, std::vector<uint16_t>& arrAddrs)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::createStringArray() : '%s:%d' : length %d of string is larger than maximum of %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex, size,
                                                                                                                                                         USER_STR_SIZE, codeLine._text.c_str());
            return -1;
        }
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::createStringArray() : '%s:%d' : CONST array initialisers missing, found %d, expecting %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex,
                                                                                                                                                                 int(arrInits.size()), int(arrAddrs.size()),
        ...
        for(int i=0; i<int(arrAddrs.size()); i++)
        {
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, strSize + 2, USER_CODE_START, _arraysStart, arrAddrs[i]))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::createStringArray() : '%s:%d' : not enough RAM for string %s of size %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex,
                                                                                                                                                    name.c_str(), strSize + 2, codeLine._text.c_str());
                return -1;
            }
        }
        ...
        if(!Memory::getFreePageRAM(Memory::FitDescending, arraySize, USER_CODE_START, _arraysStart, address, false, Memory::ParityEven)) // arrays do not need to be contained within pages
        {
            Cpu::reportError(Cpu::ComError, stderr, "Keywords::createStringArray() : '%s:%d' : not enough RAM for int array of size %d : %s\n", codeLine._moduleName.c_str(), codeLineIndex, arraySize,
                                                                                                                                                codeLine._text.c_str());
            return -1;
        }
        ...
        StringVar stringVar = {size, maxSize, address, text, name, "_" + Expression::createPaddedString(name, LABEL_TRUNC_SIZE - 1, ' '), VarStr2, -1, false, arrInits, arrAddrs, isInit};
        _stringVars.push_back(stringVar);
        index = int(_stringVars.size()) - 1;
    ...
    Expression::Numeric getString(void)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::getString() : '%s:%d' : syntax error in string '%s' : '%s'\n", _codeLineModule.c_str(), _codeLineStart, text.c_str(), _codeLineText.c_str());
        ...
        return Expression::Numeric(0, -1, true, false, false, Expression::String, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), text);
    }
    ...
    Expression::Numeric addressOf(void)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::sizeOf() : '%s:%d' : syntax error : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
            return Expression::Numeric();
        }
        ...
        if(Expression::findMatchingBrackets(varName, 0, lbra, rbra)  &&  lbra == varEnd)
        {
            ...
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::addressOf() : '%s:%d' : wrong number of indicies : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
                return Expression::Numeric();
            }
            ...
            for(int i=0; i<int(indexTokens.size()); i++)
            {
                ...
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::addressOf() : '%s:%d' : indicies must be literal expressions : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
                    return Expression::Numeric();
                }
                ...
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::addressOf() : '%s:%d' : indicies must be >= 0\n : %s", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
                    return Expression::Numeric();
                }
            }
        ...
        if(varIndex != -1)
        {
            ...
            for(int i=0; i<numIndices; i++)
            {
                ...
                if(indices[i] > dim - 1)
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::addressOf() : '%s:%d' : index %d:%d greater than array dimension %d:%d : %s\n", _codeLineModule.c_str(), _codeLineStart, i,
                                                                                                                                                       indices[i], i, dim, _codeLineText.c_str());
                    return Expression::Numeric();
                }
            }
            ...
            switch(_stringVars[strIndex]._varType)
            {
                ...
                case VarStr:
                {
                    ...
                    switch(indexTokens.size())
                    {
                        ...
                        default:
                        {
                            Cpu::reportError(Cpu::ComError, stderr, "Compiler::addressOf() : '%s:%d' : too many indices for string array '%s(%s) : %s'\n", _codeLineModule.c_str(), _codeLineStart,
                                                                                                                                                           _stringVars[strIndex]._name.c_str(),
                                                                                                                                                           params.c_str(), _codeLineText.c_str());
                            return Expression::Numeric();
                        }
                        break;
                    }
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::addressOf() : '%s:%d' : syntax error : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
            return Expression::Numeric();
        }

        bool relocatable = (labIndex > 0) ? true : false;
        return Expression::Numeric(address, -1, true, false, relocatable, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
    }
    ...
    Expression::Numeric sizeOf(void)
    {
        ...
        return Expression::Numeric(size, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
    }
    ...
    bool userFunc(const std::string& name)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::userFunc() : '%s:%d' : parenthesis error : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
            return false;
        }
        ...
        if(params.size() == 0)
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::userFunc() : '%s:%d' : syntax error, need at least one parameter : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
            return false;
        }
        int paramsSize = int(getDefFunctions()[name]._params.size());
        if(paramsSize != int(params.size()))
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::userFunc() : '%s:%d' : syntax error, wrong number of parameters, expecting %d : %s\n", _codeLineModule.c_str(), _codeLineStart,
                                                                                                                                                      paramsSize, _codeLineText.c_str());
            return false;
        }
    ...
    Expression::Numeric factor(Expression::Numeric& input, int16_t defaultValue)
    {
        double value = 0;
        Expression::Numeric numeric;
        ...
        if(Expression::find(" &("))
        {
            ...
            numeric = Compiler::expression(input); if(!numeric._isValid) return numeric;
            numeric._condCode._ccType = Expression::FastCC;

            // Parameters
            while(peek(true)  &&  peek(true) != ')')
            {
                if(get(true) == ',') numeric._params.push_back(Compiler::expression(input));
            }
            ...
            if(peek(true) != ')')
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::factor() : '%s:%d' : found '%c' expecting ')'\n", Expression::getExpressionToParse(), _codeLineStart, peek(true));
        ...
        else if((peek(true) >= '0'  &&  peek(true) <= '9')  ||  peek(true) == '&')
        {
            if(number(value))
            {
                numeric = Expression::Numeric(value, -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
            }
            else
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::factor() : '%s:%d' : syntax error in number literal : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
                numeric = Expression::Numeric();
            }
        }
        // Char literals
        else if(peek(true) == '\'')
        {
            ...
            if((chr = peek(false)) != 0) // don't skip spaces as char literal can be a space
            {
                if(chr >= 32)
                {
                    ...
                    {
                        validChar = true;
                        numeric = Expression::Numeric(double(chr), -1, true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));
            ...
            if(!validChar)
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::factor() : '%s:%d' : syntax error in character literal : %s\n", _codeLineModule.c_str(), _codeLineStart, _codeLineText.c_str());
                numeric = Expression::Numeric();
            }
        }
        // Strings
        ...
        {
            get(true); numeric = factor(input, 0); numeric = Operators::POS(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(peek(true) == '-')
        {
            get(true); numeric = factor(input, 0); numeric = Operators::NEG(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::find("NOT "))
        {
            numeric = factor(input, 0); numeric = Operators::NOT(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        ...
        {
            numeric = factor(input, 0); numeric = Functions::PEEK(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("DEEK"))
        {
            numeric = factor(input, 0); numeric = Functions::DEEK(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("USR"))
        {
            numeric = factor(input, 0); numeric = Functions::USR(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("RND"))
        {
            numeric = factor(input, 0); numeric = Functions::RND(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("URND"))
        {
            numeric = factor(input, 0); numeric = Functions::URND(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LEN"))
        {
            // Functions::IARR() needs to return an address rather than a value for LEN()
            input._returnAddress = true;
            numeric = factor(input, 0); numeric = Functions::LEN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("GET"))
        {
            numeric = factor(input, 0); numeric = Functions::GET(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ABS"))
        {
            numeric = factor(input, 0); numeric = Functions::ABS(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("SGN"))
        {
            numeric = factor(input, 0); numeric = Functions::SGN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ASC"))
        {
            numeric = factor(input, 0); numeric = Functions::ASC(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("TWIDTH"))
        {
            numeric = factor(input, 0); numeric = Functions::TWIDTH(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("STRCMP"))
        {
            numeric = factor(input, 0); numeric = Functions::STRCMP(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("BCDCMP"))
        {
            numeric = factor(input, 0); numeric = Functions::BCDCMP(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("VAL"))
        {
            numeric = factor(input, 0); numeric = Functions::VAL(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LUP"))
        {
            numeric = factor(input, 0); numeric = Functions::LUP(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ADDR"))
        {
            // Functions::IARR() needs to return an address rather than a value for ADDR()
            input._returnAddress = true;
            numeric = factor(input, 0); numeric = Functions::ADDR(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("POINT"))
        {
            numeric = factor(input, 0); numeric = Functions::POINT(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("MIN"))
        {
            numeric = factor(input, 0); numeric = Functions::MIN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("MAX"))
        {
            numeric = factor(input, 0); numeric = Functions::MAX(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("CLAMP"))
        {
            numeric = factor(input, 0); numeric = Functions::CLAMP(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("COND"))
        {
            numeric = factor(input, 0); numeric = Functions::COND(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FMUL"))
        {
            numeric = factor(input, 0); numeric = Functions::FMUL(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FDIV"))
        {
            numeric = factor(input, 0); numeric = Functions::FDIV(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FMOD"))
        {
            numeric = factor(input, 0); numeric = Functions::FMOD(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FDIVMOD"))
        {
            numeric = factor(input, 0); numeric = Functions::FDIVMOD(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("SQRT"))
        {
            numeric = factor(input, 0); numeric = Functions::SQRT(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("CHR$"))
        {
            numeric = factor(input, 0); numeric = Functions::CHR$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("SPC$"))
        {
            numeric = factor(input, 0); numeric = Functions::SPC$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("STR$"))
        {
            numeric = factor(input, 0); numeric = Functions::STR$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("STRING$"))
        {
            numeric = factor(input, 0); numeric = Functions::STRING$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("HEX$"))
        {
            numeric = factor(input, 0); numeric = Functions::HEX$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LEFT$"))
        {
            numeric = factor(input, 0); numeric = Functions::LEFT$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("RIGHT$"))
        {
            numeric = factor(input, 0); numeric = Functions::RIGHT$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("MID$"))
        {
            numeric = factor(input, 0); numeric = Functions::MID$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LCASE$"))
        {
            numeric = factor(input, 0); numeric = Functions::LCASE$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("UCASE$"))
        {
            numeric = factor(input, 0); numeric = Functions::UCASE$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("STRCAT$"))
        {
            numeric = factor(input, 0); numeric = Functions::STRCAT$(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("CEIL"))
        {
            numeric = factor(input, 0); numeric = Operators::CEIL(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("FLOOR"))
        {
            numeric = factor(input, 0); numeric = Operators::FLOOR(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("POW"))
        {
            numeric = factor(input, 0); numeric = Operators::POWF(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("SQRT"))
        {
            numeric = factor(input, 0); numeric = Operators::SQRT(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("EXP2"))
        {
            numeric = factor(input, 0); numeric = Operators::EXP2(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("EXP"))
        {
            numeric = factor(input, 0); numeric = Operators::EXP(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LOG10"))
        {
            numeric = factor(input, 0); numeric = Operators::LOG10(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LOG2"))
        {
            numeric = factor(input, 0); numeric = Operators::LOG2(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("LOG"))
        {
            numeric = factor(input, 0); numeric = Operators::LOG(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("SIN"))
        {
            numeric = factor(input, 0); numeric = Operators::SIN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("COS"))
        {
            numeric = factor(input, 0); numeric = Operators::COS(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("TAN"))
        {
            numeric = factor(input, 0); numeric = Operators::TAN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ASIN"))
        {
            numeric = factor(input, 0); numeric = Operators::ASIN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ACOS"))
        {
            numeric = factor(input, 0); numeric = Operators::ACOS(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ATAN2"))
        {
            numeric = factor(input, 0); numeric = Operators::ATAN2(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("ATAN"))
        {
            numeric = factor(input, 0); numeric = Operators::ATAN(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("RAND"))
        {
            numeric = factor(input, 0); numeric = Operators::RAND(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("REV16"))
        {
            numeric = factor(input, 0); numeric = Operators::REV16(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("REV8"))
        {
            numeric = factor(input, 0); numeric = Operators::REV8(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else if(Expression::findFunc("REV4"))
        {
            numeric = factor(input, 0); numeric = Operators::REV4(numeric, _codeLineModule, _codeLineText, _codeLineStart);
        }
        else
        {
            ...
            switch(peek(true))
            {
                ...
                case 0: numeric = Expression::Numeric(defaultValue, -1, false, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string("")); break;
                ...
                default:
                {
                    ...
                    if(varIndex != -1)
                    {
                           ...
                           _integerVars[varIndex]._varType == Var1Arr16  ||  _integerVars[varIndex]._varType == Var2Arr16  ||  _integerVars[varIndex]._varType == Var3Arr16)
                        {
                            ...
                            // Array numeric
                            numeric = Expression::Numeric(defaultValue, int16_t(varIndex), true, false, false, varType, {false, Expression::BooleanCC}, Expression::Int16Both, varName, std::string(""));
                            numeric._returnAddress = input._returnAddress;

                            // Array index parameters, (commands like LEN expect no array indices)
                            if(foundParams)
                            {
                                Expression::Numeric param = factor(input, 0);
                        ...
                        else
                        {
                            ...
                            numeric = Expression::Numeric(defaultValue, int16_t(varIndex), true, false, false, Expression::IntVar16, {false, Expression::BooleanCC}, int16Byte, varName, std::string(""));
                        }
                    }
                    // Strings
                    else if(strIndex != -1)
                    {
                        ...
                        if(_stringVars[strIndex]._varType == VarStr2)
                        {
                            ...
                            numeric = Expression::Numeric(defaultValue, int16_t(strIndex), true, false, false, Expression::Str2Var, {false, Expression::BooleanCC}, Expression::Int16Both, varName, _stringVars[strIndex]._text);
                        ...
                        {
                            // Numeric is now passed back to compiler, (rather than just numeric._value), so make sure all fields are valid
                            numeric = Expression::Numeric(defaultValue, int16_t(strIndex), true, false, false, Expression::StrVar, {false, Expression::BooleanCC}, Expression::Int16Both, varName, _stringVars[strIndex]._text);
                        }
                    }
                    // Constants
                    else if(constIndex != -1)
                    {
                        ...
                        switch(_constants[constIndex]._varType)
                        {
                            ...
                            case ConstInt16:
                            {
                                numeric = Expression::Numeric(_constants[constIndex]._data, int16_t(constIndex), true, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, varName, std::string(""));
                                if(Compiler::getConstants()[constIndex]._lazyUpdate)
                                {
                                    numeric._value = 0x0000;
                                    numeric._lazyUpdate = _constants[constIndex]._lazyUpdate;
                                }
                            }
                            break;
                            ...
                            case ConstStr:
                            {
                                numeric = Expression::Numeric(defaultValue, int16_t(constIndex), true, false, false, Expression::Constant, {false, Expression::BooleanCC}, Expression::Int16Both, varName, _constants[constIndex]._text);
                            }
                            break;
                    ...
                    {
                        numeric = Expression::Numeric(defaultValue, -1, false, false, false, Expression::Number, {false, Expression::BooleanCC}, Expression::Int16Both, std::string(""), std::string(""));

                        if(varName.size())
                        {
                            Cpu::reportError(Cpu::ComError, stderr, "\nCompiler::factor() : '%s:%d' : found an unknown symbol '%s' : %s\n", _codeLineModule.c_str(), _codeLineStart, varName.c_str(), _codeLineText.c_str());
                        }
                        else
                        {
                            Expression::advance(-1);
                            Cpu::reportError(Cpu::ComError, stderr, "\nCompiler::factor() : '%s:%d' : found an unknown symbol '%s' : %s\n", _codeLineModule.c_str(), _codeLineStart,
                                                                                                                                            Expression::getExpression(), _codeLineText.c_str());
                            Expression::advance(1);
                        }
                    }
                }
    ...
    Expression::Numeric term(Expression::Numeric& input)
    {
        Expression::Numeric init, result = factor(input, 0);
        if(!result._isValid) return result;

        for(;;)
        {
            if(Expression::find("**"))       {init = factor(input, 0); if(!init._isValid) return init; result = Operators::POW(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('*'))               {init = factor(input, 0); if(!init._isValid) return init; result = Operators::MUL(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('/'))               {init = factor(input, 0); if(!init._isValid) return init; result = Operators::DIV(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('%'))               {init = factor(input, 0); if(!init._isValid) return init; result = Operators::MOD(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("MOD")) {init = factor(input, 0); if(!init._isValid) return init; result = Operators::MOD(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else return result;
        }
    }

    Expression::Numeric expr(Expression::Numeric& input)
    {
        Expression::Numeric init, result = term(input);
        if(!result._isValid) return result;

        for(;;)
        {
            // Toggle string work area between lhs and rhs for '+', (no need to check that parameters are strings)
            if(find('+'))      {init = term(input); if(!init._isValid) return init; result = Operators::ADD(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('-')) {init = term(input); if(!init._isValid) return init; result = Operators::SUB(result, init, _codeLineModule, _codeLineText, _codeLineStart);}

            else return result;
        }
    }

    Expression::Numeric logical(Expression::Numeric& input)
    {
        Expression::Numeric init, result = expr(input);
        if(!result._isValid) return result;

        for(;;)
        {
            // Boolean conditionals
            if(Expression::find("=="))      {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::EQ(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('='))              {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::EQ(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("<>")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::NE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("<=")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::LE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find(">=")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::GE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('<'))              {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::LT(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(find('>'))              {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::BooleanCC; result = Operators::GT(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}

            // Normal conditionals
            else if(Expression::find("&==")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::EQ(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&="))  {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::EQ(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&<>")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::NE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&<=")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::LE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&>=")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::GE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&<"))  {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::LT(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&>"))  {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::NormalCC; result = Operators::GT(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}

            // Fast conditionals
            else if(Expression::find("&&==")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::EQ(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&&="))  {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::EQ(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&&<>")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::NE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&&<=")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::LE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&&>=")) {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::GE(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&&<"))  {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::LT(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("&&>"))  {init = expr(input); if(!init._isValid) return init; input._condCode._ccType = Expression::FastCC; result = Operators::GT(result, init, input, _codeLineModule, _codeLineText, _codeLineStart);}

            else return result;
        }
    }

    Expression::Numeric expression(Expression::Numeric& input)
    {
        Expression::Numeric init, result = logical(input);
        if(!result._isValid) return result;

        for(;;)
        {
            if(Expression::find("AND"))      {init = logical(input); if(!init._isValid) return init; result = Operators::AND(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("XOR")) {init = logical(input); if(!init._isValid) return init; result = Operators::XOR(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("OR"))  {init = logical(input); if(!init._isValid) return init; result = Operators::OR(result,  init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("LSL")) {init = logical(input); if(!init._isValid) return init; result = Operators::LSL(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("LSR")) {init = logical(input); if(!init._isValid) return init; result = Operators::LSR(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
            else if(Expression::find("ASR")) {init = logical(input); if(!init._isValid) return init; result = Operators::ASR(result, init, _codeLineModule, _codeLineText, _codeLineStart);}
    ...
    StrResult assignString(CodeLine& codeLine, int codeLineIndex, int codeLineStart, Expression::Numeric& numeric, uint32_t expressionType)
    {
        ...
        if(dstIndex == -1)
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::assignString() : '%s:%d' : syntax error : %s\n", codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
            return StrError;
        }
    ...
    bool assignInt(CodeLine& codeLine, int codeLineIndex, int codeLineStart, Expression::Numeric& numeric, uint32_t expressionType, int varIndexRhs)
    {
        if(!_assignVar)
        {
            _assignVar = true;
        ...
        if(codeLine._varIndex != -1)
        {
            ...
            if(codeLine._containsVars)
            {
                ...
                {
                    if(!writeArrayVar(codeLine, codeLineIndex, codeLine._varIndex))
                    {
                        Cpu::reportError(Cpu::ComError, stderr, "Compiler::assignInt() : '%s:%d' : syntax error trying to write to array : %s\n", codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
                        return false;
                    }
                }
                else
            ...
            else
            {
                ...
                if(!(expressionType & Expression::HasFunctions)  ||  numeric._varType == Expression::Number)
                {
                    ...
                    // 8bit constants
                    else if(_integerVars[codeLine._varIndex]._data >=0  &&  _integerVars[codeLine._varIndex]._data <= 255)
                    {
    ...
    StatementResult createVasmCode(CodeLine& codeLine, int codeLineIndex, int codeLineStart)
    {
        ...
        if(!_codeLines[_currentCodeLineIndex]._pushEmitted  &&  codeLine._labelIndex >= 0  &&  _labels[codeLine._labelIndex]._gosub)
        {
            ...
            // Programmer can manually control emitting of PUSH through NO_PUSH modifier
            if(!_labels[codeLine._labelIndex]._noPush) emitVcpuAsm("PUSH", "", false, codeLineIndex);
        }
        ...
        for(int i=0; i<int(codeLine._tokens.size()); i++)
        {
            ...
            if(keywordResult == Keywords::KeywordError) return StatementError; // keyword handlers print their own error messages
            ...
            if(Keywords::getKeywords().find(token) != Keywords::getKeywords().end())
            {
                ...
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::createVasmCode() : '%s:%d' : syntax error in : '%s' : %s\n", codeLine._moduleName.c_str(), codeLineStart,
                                                                                                                                    codeLine._tokens[0].c_str(), codeLine._text.c_str());
        ...
        if(expressionType == Expression::IsInvalid)
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::createVasmCode() : '%s:%d' : expression error in : '%s' : %s\n", codeLine._moduleName.c_str(), codeLineStart,
                                                                                                                                codeLine._expression.c_str(), codeLine._text.c_str());
            return StatementError;
        }
        ...
        if(codeLine._varIndex != -1)
        {
            ...
            numeric = Expression::Numeric(0, int16_t(codeLine._varIndex), true, false, false, varType, {false, Expression::BooleanCC}, Expression::Int16Both, name, std::string(""));
        }
        if(!Expression::parse(codeLine._expression, codeLineIndex, numeric))
        {
            Cpu::reportError(Cpu::ComError, stderr, "Compiler::createVasmCode() : '%s:%d' : syntax error in : '%s'\n", codeLine._moduleName.c_str(), codeLineStart, Expression::getExpression());
            return StatementError;
        }
        ...
        // Int assignment
        if(!assignInt(codeLine, codeLineIndex, codeLineStart, numeric, expressionType, varIndexRhs)) return StatementError;

        return StatementExpression;
    }

    // Very simple check, if the programmer goes out of his way to write gnarly code, e.g. ()()()()()()(((var((()()))))), then they are on their own
...
REDO_STATEMENT:
            ...
            if(!createCodeLine(tokens[j], 0, codeLine._labelIndex, -1, Expression::Int16Both, false, codeLine, codeLine._moduleName))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::parseMultiStatements() : '%s:%d' : createCodeLine() failed in : '%s'\n", codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
                return StatementError;
            }
            ...
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::parseMultiStatements() : '%s:%d' : syntax error, brackets do not match : %s\n", codeLine._moduleName.c_str(), codeLineStart, codeLine._text.c_str());
                return StatementError;
            }
            ...
            // Create vars
            if(createCodeVar(codeLine, codeLineIndex, varIndex) == VarIsDifferentType) return StatementError;
            ...
            // Some commands, (such as FN), modify the BASIC source and cause a re-evaluation of the current tokens
            if(statementResult == RedoStatementParse) goto REDO_STATEMENT;

            // Some commands, (such as IF), process multi-statements themselves
            if(statementResult == MultiStatementParsed) return MultiStatementParsed;;
        }

        return statementResult;
    }

    int addLabelToJumpCC(std::vector<VasmLine>& vasmLine, const std::string& label)
    {
        for(int i=0; i<int(vasmLine.size()); i++)
        {
            if(vasmLine[i]._code.substr(0, sizeof("Jump")-1) == "Jump"  ||  _branchTypes.find(vasmLine[i]._code.substr(0, 3)) != _branchTypes.end())
            {
                vasmLine[i]._code += label;
                vasmLine[i]._operand = label;
                return i;
            }
        }
    ...
    int addLabelToJump(std::vector<VasmLine>& vasmLine, const std::string& label)
    {
        for(int i=0; i<int(vasmLine.size()); i++)
        {
            if(_codeRomType >= Cpu::ROMv5a)
            {
                if(vasmLine[i]._code.find("CALLI_JUMP") != std::string::npos)
                {
                    vasmLine[i]._opcode = "CALLI";
                    vasmLine[i]._operand = label;
                    vasmLine[i]._code = Expression::createPaddedString("CALLI", OPCODE_TRUNC_SIZE, ' ') + label;
                    return i;
                }
            }
            else
            {
                if(vasmLine[i]._code.find("BRA_JUMP") != std::string::npos)
                {
                    vasmLine[i]._opcode = "BRA";
                    vasmLine[i]._operand = label;
                    vasmLine[i]._code = Expression::createPaddedString("BRA", OPCODE_TRUNC_SIZE, ' ') + label;
                    return i;
                }
                else if(vasmLine[i]._code.find("LDWI_JUMP") != std::string::npos)
                {
                    vasmLine[i]._opcode = "LDWI";
                    vasmLine[i]._operand = label;
                    vasmLine[i]._code = Expression::createPaddedString("LDWI", OPCODE_TRUNC_SIZE, ' ') + label;
                    return i;
                }
            }
        }
    ...
    {
        // Pragma defines
        for(const auto& pd : _pragmaDefines)
        {
            emitVcpuPreProcessingCmd(pd);
        }
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            ...
            if(i > 0  &&  _codeLines[i]._code.size() >= 2)
            {
                ...
                if(statementResult == StatementError)
                {
                    if(Cpu::getCriticalError() <= Cpu::WarnError)
                    {
                        Cpu::reportError(Cpu::XxxError, stderr, "Compiler::parseCode() : '%s:%d' : unknown error in : %s\n*** This error message is an internal debugging message, it should NOT appear, please contact me ***",
                                                                _codeLines[i]._moduleName.c_str(), codeLineStart, _codeLines[i]._text.c_str());
                    }
                    return false;
                }

                // Enable/disable optimiser
                _codeLines[i]._optimiserEnabled = _enableOptimiser;
            }
        }
        ...
        for(int i=0; i<int(_codeLines[0]._vasm.size()); i++)
        {
            for(int j=0; j<int(_sysInitNames.size()); j++)
            {
                if(_codeLines[0]._vasm[i]._opcode == _sysInitNames[j])
                {
                    ...
                    adjustLabelAddresses(address, SYS_INIT_FUNC_LEN);
                    adjustVasmAddresses(0, address, SYS_INIT_FUNC_LEN);
    ...
    void outputReservedWords(void)
    {
        ...
        Expression::addString(line, LABEL_TRUNC_SIZE - int(line.size()));
        line += Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_userCodeStart) + "\n";
        _output.push_back(line);
    }

    // TODO: fix this hack, (shouldn't be extracting addresses from names or be doing any string processing here!)
    bool getAddressFromName(const std::string& labelName, uint16_t& address)
    {
        if(labelName.size() <= 7) return false;

        std::string addrStr = labelName.substr(labelName.size() - 7);
        if(addrStr.substr(0, 3) != "_0x") return false;

        addrStr = addrStr.substr(3);
        if(!Expression::isHex(addrStr)) return false;
    ...
    void outputLabels(void)
    {
        ...
        // BASIC and VASM labels
        for(int i=0; i<int(_labels.size()); i++)
        {
            std::string addrStr = Expression::wordToHexString(_labels[i]._address);
            _output.push_back(_labels[i]._output + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + addrStr + "\n");
    ...
    void outputConsts(void)
    {
        ...
        for(int i=0; i<int(_constants.size()); i++)
        {
            ...
            int16_t* lazyUpdate = _constants[i]._lazyUpdate;
            if(lazyUpdate) continue;

            switch(constType)
            {
                case ConstInt16:
                {
                    _output.push_back(Expression::createPaddedString(internalName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(data) + "\n");
                }
                break;
    ...
    void outputLazyConsts(void)
    {
        _output.push_back("; Lazy Constants\n");
    ...
    void outputVars(void)
    {
        ...
        {
            if(_integerVars[varIndex]._address >= LOCAL_VAR_START) continue;
            ...
            switch(_integerVars[varIndex]._varType)
            {
                case VarInt16:
                {
                    ...
                    _output.push_back(_integerVars[varIndex]._output + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + address + "\n");
                }
                break;
    ...
    void outputArrs(void)
    {
        ...
        for(int varIndex=0; varIndex<int(_integerVars.size()); varIndex++)
        {
            switch(_integerVars[varIndex]._varType)
            {
                case Var1Arr8:
                {
                    std::string arrName = "_" + _integerVars[varIndex]._name;
                    _output.push_back(Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_integerVars[varIndex]._arrAddrs[0][0]) + "\n");

                    // Don't output DB statements for an uninitialised array
                    if(_integerVars[varIndex]._arrInit)
                    {
                        std::string dbString = Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
                ...
                case Var1Arr16:
                {
                    ...
                    if(_integerVars[varIndex]._arrInit)
                    {
                        std::string dwString = Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                ...
                {
                    std::string arrName = "_" + _integerVars[varIndex]._name;
                    _output.push_back(Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_integerVars[varIndex]._address) + "\n");
                    std::string dwString = Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                    ...
                    for(int j=0; j<_integerVars[varIndex]._arrSizes[1]; j++)
                    {
                        ...
                        _output.push_back(Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_integerVars[varIndex]._arrAddrs[0][j]) + "\n");
                ...
                case Var2Arr16:
                {
                    ...
                    for(int j=0; j<_integerVars[varIndex]._arrSizes[1]; j++)
                    {
                        ...
                        if(_integerVars[varIndex]._arrInit)
                        {
                            dwString = Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                ...
                case Var3Arr8:
                {
                    ...
                    for(int k=0; k<_integerVars[varIndex]._arrSizes[0]; k++)
                    {
                        ...
                        _output.push_back(Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_integerVars[varIndex]._arrLut[k]) + "\n");
                        dwString = Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                    ...
                    for(int k=0; k<_integerVars[varIndex]._arrSizes[0]; k++)
                    {
                        ...
                        for(int j=0; j<_integerVars[varIndex]._arrSizes[1]; j++)
                        {
                            ...
                            _output.push_back(Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_integerVars[varIndex]._arrAddrs[k][j]) + "\n");
    ...
    bool sanitiseString(const std::string& input, std::string& output, int& length)
    {
        ...
        {
            Cpu::reportError(Cpu::ComError, stderr, "Expression::sanitiseString() : string '%s' of size '%d' is larger than '%d' chars\n", output.c_str(), int(output.size()), USER_STR_SIZE);
            return false;
        }
    ...
    bool outputStrs(void)
    {
        ...
        for(int i=0; i<int(_stringVars.size()); i++)
        {
            ...
            if(_stringVars[i]._varType == VarStr  &&  !_stringVars[i]._constant)
            {
                ...
                _output.push_back(_stringVars[i]._output + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_stringVars[i]._address) + "\n");
                _output.push_back(_stringVars[i]._output + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ') + std::to_string(len) + " '" + str + "' 0\n");
            }
            // Array of strings
            else if(_stringVars[i]._varType == VarStr2)
            {
                std::string arrName = "_" + _stringVars[i]._name;
                _output.push_back(Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_stringVars[i]._address) + "\n");
                std::string dwString = Expression::createPaddedString(arrName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                ...
                for(int j=0; j<int(_stringVars[i]._arrAddrs.size()); j++)
                {
                    ...
                    _output.push_back(Expression::createPaddedString(strName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_stringVars[i]._arrAddrs[j]) + "\n");
                    ...
                    if(_stringVars[i]._arrInit)
                    {
                        ...
                        _output.push_back(Expression::createPaddedString(strName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ') + std::to_string(len) + " '" + str + "' 0\n");
                    }
                }
            }
        }
    ...
    bool outputDATA(void)
    {
        ...
        for(int i=0; i<int(_dataObjects.size()); i++)
        {
            ...
            switch(pObject->_dataType)
            {
                case DataInteger:
                {
                    ...
                    int size = 2;
                    if(!Memory::getFreePageRAM(Memory::FitDescending, size, USER_CODE_START, _runtimeStart, pData->_address, true))
                    {
                        Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputDATA() : not enough RAM for data of size %d\n", size);
                        return false;
                    }
                    std::string defName = "_data_" + Expression::wordToHexString(pData->_address);
                    _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(pData->_address) + "\n");
                    _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ') + std::to_string(var) + "\n");
                }
                break;
                ...
                case DataString:
                {
                    ...
                    std::string defName = "_data_" + Expression::wordToHexString(pData->_address);
                    _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(pData->_address) + "\n");
                    _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ') + std::to_string(size-2) + " '" + str + "' 0\n");
                }
                break;
        ...
        if(_dataObjects.size())
        {
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, size, USER_CODE_START, _runtimeStart, address, false, Memory::ParityEven))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputDATA() : not enough RAM for data LUT of size %d\n", size);
                return false;
            }
            std::string defName = "_data_";
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(address) + "\n");
            std::string dwString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<int(_dataObjects.size()); i++)
            {
                DataObject* pObject = _dataObjects[i].get();
            ...
            address = 0x0000;
            size = 2;
            if(!Memory::getFreePageRAM(Memory::FitDescending, size, USER_CODE_START, _runtimeStart, address, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputDATA() : not enough RAM for data LUT of size %d\n", size);
                return false;
            }
            defName = "_dataIndex_";
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(address) + "\n");
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ') + "0\n");
        }
    ...
    bool outputTIME(void)
    {
        ...
        if(_createTimeData)
        {
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, timeArraySize, USER_CODE_START, _runtimeStart, timeArrayAddress, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputTIME() : not enough RAM for time array of size %d\n", timeArraySize);
                return false;
            }
            std::string defName = "_timeArray_";
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(timeArrayAddress) + "\n");
            std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
            _output.push_back(dbString + "00 00 00\n");

            int timeStringSize = 10;
            uint16_t timeStringAddress;
            if(!Memory::getFreePageRAM(Memory::FitDescending, timeStringSize, USER_CODE_START, _runtimeStart, timeStringAddress, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputTIME() : not enough RAM for time string of size %d\n", timeStringSize);
                return false;
            }
            defName = "_timeString_";
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(timeStringAddress) + "\n");
            dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
    ...
    bool outputMidiDef(int midiId, uint16_t startAddr, uint16_t jmpAddr, int size, int& dataIndex)
    {
        ...
        _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(startAddr) + "\n");
        std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
    ...
    bool outputBlitDef(int blitId, int numStripeChunks, uint16_t address, int& dataIndex)
    {
        std::string defName = "def_blits_" + Expression::wordToHexString(address);
        _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(address) + "\n");
        std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
        ...
        {
            for(int j=0; j<BLIT_CHUNK_SIZE; j++)
            {
                dbString += std::to_string(_defDataBlits[blitId]._data[dataIndex++]) + " ";
            }
        }
    ...
    bool outputSpriteDef(int spriteId, uint16_t address, int& dataIndex)
    {
        std::string defName = "def_sprites_" + Expression::wordToHexString(address);
        _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(address) + "\n");
        std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');

        // Output a scanline of sprite data, (oldX, OldY, back0, data0 ... back8, data8)
        for(int i=0; i<SPRITE_WIDTH*2 + 2; i++)
        {
            if(i >= 3  &&  (i & 1))
            {
    ...
    bool outputPatternDef(int patternId, uint16_t address, int& dataIndex)
    {
        std::string defName = "def_patterns_" + Expression::wordToHexString(address);
        _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(address) + "\n");
        std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');

        // Output two scanlines of pattern data
        for(int i=0; i<SPRITE_WIDTH*2; i++)
        {
            dbString += std::to_string(_defDataPatterns[patternId]._data[dataIndex++]) + " ";
        }
        _output.push_back(dbString + "\n");
    ...
    bool outputDefs(void)
    {
        ...
        for(int i=0; i<int(_defDataBytes.size()); i++)
        {
            ...
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_defDataBytes[i]._address) + "\n");
        ...
        for(int i=0; i<int(_defDataWords.size()); i++)
        {
            ...
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_defDataWords[i]._address) + "\n");
        ...
        for(int i=0; i<int(_defDataLoaderImageChunks.size()); i++)
        {
            ...
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(srcAddr) + "\n");
        ...
        // Create def blit data
        _output.push_back("; Define Blits\n");
        for(auto it=_defDataBlits.begin(); it!=_defDataBlits.end(); ++it)
        {
            int blitId = it->first;
            ...
            {
                Cpu::reportError(Cpu::WarnError, stderr, "Compiler::outputDefs() : Warning blit %d is missing stripe addresses\n", blitId);
                continue;
            }
                ...
                for(int j=0; j<numColumns; j++)
                {
                    ...
                    if(numStripesPerCol > 1)
                    {
                        for(int k=0; k<numStripesPerCol-1; k++)
                        {
                            ...
                            outputBlitDef(blitId, numStripeChunks, address, dataIndex);
                        }
                        uint16_t address = it->second._stripeAddrs[j*numStripesPerCol*2 + (numStripesPerCol-1)*2];
                        outputBlitDef(blitId, remStripeChunks, address, dataIndex);
                    }
        ...
        if(_defDataFonts.size() > 0)
        {
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, fontIdSize, USER_CODE_START, _runtimeStart, fontIdAddress))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputDefs() : not enough RAM for font id var of size %d\n", fontIdSize);
                return false;
            }
            std::string defName = "_fontId_";
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(fontIdAddress) + "\n");
            std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
        ...
        for(auto it=_defDataFonts.begin(); it!=_defDataFonts.end(); ++it)
        {
            ...
            {
                Cpu::reportError(Cpu::WarnError, stderr, "Compiler::outputDefs() : warning font %d is missing mapping table\n", fontId);
                continue;
            }
        ...
        for(auto it=_defDataSprites.begin(); it!=_defDataSprites.end(); ++it)
        {
            ...
            int height = it->second._height;
            uint16_t address = it->second._address;

            // For each column of sprite data
            int dataIndex = 0;
            for(int j=0; j<height; j++)
            {
                outputSpriteDef(spriteId, address + uint16_t(j) * 0x0100, dataIndex);
            }
        }
        _output.push_back("\n");

        // Create def pattern data
        _output.push_back("; Define Patterns\n");
        for(auto it=_defDataPatterns.begin(); it!=_defDataPatterns.end(); ++it)
        {
    ...
    bool outputLuts(void)
    {
        ...
        if(_createNumericLabelLut)
        {
            ...
            for(int i=0; i<int(_labels.size()); i++)
            {
                if(_labels[i]._numeric)
                {
                    ...
                    {
                        Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : bad numeric label %s : %04x\n", _labels[i]._name.c_str(), numericLabel);
                        return false;
                    }
            ...
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : There is a mismatch in Numeric labels for one or more GOTO <var> and GOSUB <var>\n");
                Cpu::reportError(Cpu::ComError, stderr, "                         Correct your line numbers and add appropriate trailers, ':' for GOTO <var> and '!' for GOSUB <var>\n");
                return false;
            }
            else
            {
                ...
                uint16_t lutAddress;
                if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize + 2, USER_CODE_START, _runtimeStart, lutAddress, true))
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for numeric labels LUT of size %d\n", lutSize + 2);
                    return false;
                }

                std::string lutName = "_lut_numericLabs";
                _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(lutAddress) + "\n");

                std::string dwString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                ...
                if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, lutAddress, true))
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for numeric addresses LUT of size %d\n", lutSize);
                    return false;
                }
                ...
                dwString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            ...
            if(lutSize)
            {
                ...
                for(int j=0; j<lutSize; j++)
                {
                    ...
                    if(index == -1) Cpu::reportError(Cpu::WarnError, stderr, "Compiler::outputLuts() : warning label index is invalid for LUT entry %d\n", j);
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            ...
            if(varsSize)
            {
                _output.push_back(Expression::createPaddedString(varsName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(varsAddr) + "\n");

                std::string dwString = Expression::createPaddedString(varsName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            ...
            if(strsSize)
            {
                _output.push_back(Expression::createPaddedString(strsName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(strsAddr) + "\n");

                std::string dwString = Expression::createPaddedString(strsName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            ...
            if(typesSize)
            {
                _output.push_back(Expression::createPaddedString(typesName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(typesAddr) + "\n");

                std::string dwString = Expression::createPaddedString(typesName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            ...
            if(varsSize  &&  strsSize  &&  typesSize  &&  _codeLines[i]._inputLut._address)
            {
                ...
                _output.push_back(Expression::createPaddedString(name, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(lutAddress) + "\n");

                std::string dwString = Expression::createPaddedString(name, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
        ...
        if(_defDataLoaderImageChunks.size())
        {
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, lutAddress, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputDefs() : not enough RAM for loader image LUT of size %d\n", lutSize);
                return false;
            }

            // Loader images LUT delimited by 0
            std::string defName = "_loader_image_chunksLut";
            _output.push_back(Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(lutAddress) + "\n");
            std::string dbString = Expression::createPaddedString(defName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<int(_defDataLoaderImageChunks.size()); i++)
            {
                uint16_t srcAddr = _defDataLoaderImageChunks[i]._lutEntry._srcAddr;
        ...
        if(_defDataMidis.size())
        {
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, lutAddress, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for midis LUT of size %d\n", lutSize);
                return false;
            }
        ...
        for(auto it=_defDataBlits.begin(); it!=_defDataBlits.end(); ++it)
        {
            ...
            if(it->second._stripeAddrs.size() == 0)
            {
                Cpu::reportError(Cpu::WarnError, stderr, "Compiler::outputLuts() : warning blit %d is missing stripe addresses\n", blitId);
                continue;
            }

            // Allocate RAM for blit addresses/offsets and delimiter
            uint16_t lutAddress;
            int lutSize = int(it->second._stripeAddrs.size())*2 + 2;
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, lutAddress, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for blit %d address LUT of size %d\n", blitId, lutSize);
                return false;
            }
            _blitsAddrLut._addrs.push_back(lutAddress);

            std::string lutName = "_blitLut_" + Expression::wordToHexString(lutAddress);
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(lutAddress) + "\n");
        ...
        // BLITS LUT
        if(_blitsAddrLut._addrs.size())
        {
            // Allocate RAM for blits LUT
            if(_blitsAddrLutAddress == 0x0000)
            {
                int lutSize = int(_blitsAddrLut._addrs.size()) * 2;
                if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _blitsAddrLutAddress, true))
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for blits LUT of size %d\n", lutSize);
                    return false;
                }
            }
            _blitsAddrLut._address = _blitsAddrLutAddress;

            std::string lutName = "_blitsLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_blitsAddrLutAddress) + "\n");

            std::string dwString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<int(_blitsAddrLut._addrs.size()); i++)
            {
                uint16_t address = _blitsAddrLut._addrs[i];
                dwString += Expression::wordToHexString(address) + " ";
            }
            _output.push_back(dwString + "\n");
        }
        ...
        for(auto it=_defDataFonts.begin(); it!=_defDataFonts.end(); ++it)
        {
            ...
            {
                Cpu::reportError(Cpu::WarnError, stderr, "Compiler::outputLuts() : warning font %d is missing char addresses\n", fontId);
                continue;
            }
            ...
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, lutAddress, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for font %d address LUT of size %d\n", fontId, lutSize);
                return false;
            }
            _fontsAddrLut._addrs.push_back(lutAddress);
        ...
        // FONTS LUT
        if(_fontsAddrLut._addrs.size())
        {
            // Allocate RAM for fonts addresses
            int lutSize = int(_fontsAddrLut._addrs.size()) * 2;
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _fontsAddrLut._address, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for fonts LUT of size %d\n", lutSize);
                return false;
            }

            std::string lutName = "_fontsLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_fontsAddrLut._address) + "\n");

            std::string dwString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<int(_fontsAddrLut._addrs.size()); i++)
            {
                uint16_t address = _fontsAddrLut._addrs[i];
                dwString += Expression::wordToHexString(address) + " ";
            }
            _output.push_back(dwString + "\n");
        }
        ...
        {
            uint16_t address = it->second._address;
            _spritesAddrLut._addrs.push_back(uint8_t(HI_BYTE(address)));
        }

        // SPRITES LUT's, contains one byte addresses, (high byte)
        if(_spritesAddrLut._addrs.size())
        {
            // Allocate RAM for sprites LUT
            int lutSize = int(_spritesAddrLut._addrs.size());
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._address, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprites LUT of size %d\n", lutSize);
                return false;
            }

            std::string lutName = "_spritesLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._address) + "\n");

            std::string dbString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<lutSize; i++)
            {
                uint8_t address = _spritesAddrLut._addrs[i];
                dbString += Expression::byteToHexString(address) + " ";
            }
            _output.push_back(dbString + "\n");

            // Allocate RAM for sprites tmp LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize*4, USER_CODE_START, _runtimeStart, _spritesAddrLut._tmpAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite tmp LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesTmpLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._tmpAddr) + "\n");

            dbString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
            ...
            // Allocate RAM for sprites indices LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._indicesAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite indices LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesIndicesLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._indicesAddr) + "\n");

            dbString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<lutSize; i++)
            {
                dbString += std::to_string(i) + " ";
            }
            _output.push_back(dbString + "\n");


            // Allocate RAM for sprites x position LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._xPosAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite x positions LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesXposLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._xPosAddr) + "\n");
            ...
            // Allocate RAM for sprites x position tmp LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._xPosTmpAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite x positions tmp LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesXposTmpLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._xPosTmpAddr) + "\n");
            ...
            // Allocate RAM for sprites y position LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._yPosAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite y positions LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesYposLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._yPosAddr) + "\n");
            ...
            // Allocate RAM for sprites y position tmp LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._yPosTmpAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite y positions tmp LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesYposTmpLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._yPosTmpAddr) + "\n");
            ...
            // Allocate RAM for sprites height LUT
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _spritesAddrLut._heightAddr, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for sprite heights LUT of size %d\n", lutSize);
                return false;
            }

            lutName = "_spritesHeightLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_spritesAddrLut._heightAddr) + "\n");

            dbString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DB", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<lutSize; i++)
            {
                dbString += std::to_string(_defDataSprites[i]._height) + " ";
            }
            _output.push_back(dbString + "\n");
        }
        ...
        for(auto it=_defDataPatterns.begin(); it!=_defDataPatterns.end(); ++it)
        {
            PatternAddrLut patternAddrLut;
            patternAddrLut._id = it->second._id;

            std::vector<uint16_t> addrs = it->second._addrs;
            for(int i=0; i<int(addrs.size()); i++)
            {
                patternAddrLut._addrs.push_back(addrs[i]);
            }

            // PATTERN LUTs
            if(patternAddrLut._addrs.size())
            {
                // Allocate RAM for patterns LUT
                int lutSize = int(patternAddrLut._addrs.size()) * 2;
                if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, patternAddrLut._address, true))
                {
                    Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for pattern %d addrs LUT of size %d\n", patternAddrLut._id, lutSize);
                    return false;
                }
                _patternAddrLuts[patternAddrLut._id] = patternAddrLut;
                _patternsAddrLut._addrs.push_back(patternAddrLut._address);

                std::string lutName = "_patternLut_" + std::to_string(patternAddrLut._id) + "_";
                _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(patternAddrLut._address) + "\n");

                std::string dwString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
                for(int i=0; i<int(patternAddrLut._addrs.size()); i++)
                {
                    uint16_t address = patternAddrLut._addrs[i];
                    dwString += Expression::wordToHexString(address) + " ";
                }
                _output.push_back(dwString + "\n");
            }
        ...
        // PATTERNS LUT
        if(_patternsAddrLut._addrs.size())
        {
            // Allocate RAM for patterns LUT
            int lutSize = int(_patternsAddrLut._addrs.size()) * 2;
            if(!Memory::getFreePageRAM(Memory::FitDescending, lutSize, USER_CODE_START, _runtimeStart, _patternsAddrLut._address, true))
            {
                Cpu::reportError(Cpu::ComError, stderr, "Compiler::outputLuts() : not enough RAM for patterns LUT of size %d\n", lutSize);
                return false;
            }

            std::string lutName = "_patternsLut_";
            _output.push_back(Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(_patternsAddrLut._address) + "\n");

            std::string dwString = Expression::createPaddedString(lutName, LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("DW", OPCODE_TRUNC_SIZE, ' ');
            for(int i=0; i<int(_patternsAddrLut._addrs.size()); i++)
            {
                uint16_t address = _patternsAddrLut._addrs[i];
                dwString += Expression::wordToHexString(address) + " ";
            }
            _output.push_back(dwString + "\n");
        }
    ...
    void outputInternalEquates(void)
    {
        ...
        _output.push_back(Expression::createPaddedString("serialRawPrev",  LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(SER_RAW_PREV)      + "\n");
        _output.push_back(Expression::createPaddedString("frameCountPrev", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(FRAME_COUNT_PREV)  + "\n");
        _output.push_back(Expression::createPaddedString("register0",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(INT_VAR_START)     + "\n");
        _output.push_back(Expression::createPaddedString("register1",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x02\n");
        _output.push_back(Expression::createPaddedString("register2",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x04\n");
        _output.push_back(Expression::createPaddedString("register3",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x06\n");
        _output.push_back(Expression::createPaddedString("register4",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x08\n");
        _output.push_back(Expression::createPaddedString("register5",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x0A\n");
        _output.push_back(Expression::createPaddedString("register6",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x0C\n");
        _output.push_back(Expression::createPaddedString("register7",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x0E\n");
        _output.push_back(Expression::createPaddedString("register8",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x10\n");
        _output.push_back(Expression::createPaddedString("register9",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x12\n");
        _output.push_back(Expression::createPaddedString("register10",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x14\n");
        _output.push_back(Expression::createPaddedString("register11",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x16\n");
        _output.push_back(Expression::createPaddedString("register12",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x18\n");
        _output.push_back(Expression::createPaddedString("register13",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x1A\n");
        _output.push_back(Expression::createPaddedString("register14",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x1C\n");
        _output.push_back(Expression::createPaddedString("register15",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x1E\n");
        _output.push_back(Expression::createPaddedString("fgbgColour",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x20\n");
        _output.push_back(Expression::createPaddedString("cursorXY",       LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x22\n");
        _output.push_back(Expression::createPaddedString("midiStream",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x24\n");
        _output.push_back(Expression::createPaddedString("midiDelay",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x26\n");
        _output.push_back(Expression::createPaddedString("miscFlags",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x28\n");
        _output.push_back(Expression::createPaddedString("register16",     LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x2A\n");
        if(_codeRomType < Cpu::ROMvX0  ||  _codeRomType >= Cpu::SDCARD)
        {
            _output.push_back(Expression::createPaddedString("timerTick",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x2C\n");
            _output.push_back(Expression::createPaddedString("timerJiff",      LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + "register0 + 0x2E\n");
        }
        _output.push_back("\n");

        _output.push_back("; Internal Constants\n");
        _output.push_back(Expression::createPaddedString("MISC_WAIT_VBLANK_BIT",   LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_WAIT_VBLANK_BIT)   + "\n");
        _output.push_back(Expression::createPaddedString("MISC_ENABLE_SCROLL_BIT", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_ENABLE_SCROLL_BIT) + "\n");
        _output.push_back(Expression::createPaddedString("MISC_ON_BOTTOM_ROW_BIT", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_ON_BOTTOM_ROW_BIT) + "\n");
        _output.push_back(Expression::createPaddedString("MISC_DISABLE_CLIP_BIT",  LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_DISABLE_CLIP_BIT)  + "\n");
        _output.push_back(Expression::createPaddedString("MISC_ENABLE_FNT4X6_BIT", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_ENABLE_FNT4X6_BIT) + "\n");
        _output.push_back(Expression::createPaddedString("MISC_WAIT_VBLANK_MSK",   LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_WAIT_VBLANK_MSK)   + "\n");
        _output.push_back(Expression::createPaddedString("MISC_ENABLE_SCROLL_MSK", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_ENABLE_SCROLL_MSK) + "\n");
        _output.push_back(Expression::createPaddedString("MISC_ON_BOTTOM_ROW_MSK", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_ON_BOTTOM_ROW_MSK) + "\n");
        _output.push_back(Expression::createPaddedString("MISC_DISABLE_CLIP_MSK",  LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_DISABLE_CLIP_MSK)  + "\n");
        _output.push_back(Expression::createPaddedString("MISC_ENABLE_FNT4X6_MSK", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::byteToHexString(MISC_ENABLE_FNT4X6_MSK) + "\n");
        _output.push_back("\n");

        _output.push_back("; Internal Buffers\n");
        _output.push_back(Expression::createPaddedString("regsWorkArea", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(getRegWorkArea()) + "\n");
        _output.push_back(Expression::createPaddedString("textWorkArea", LABEL_TRUNC_SIZE, ' ') + Expression::createPaddedString("EQU", OPCODE_TRUNC_SIZE, ' ') + Expression::wordToHexString(getStrWorkArea()) + "\n");
        _output.push_back("\n");
    }
    ...
    void outputIncludes(void)
    {
        ...
        _output.push_back(Expression::createPaddedString("%includePath", LABEL_TRUNC_SIZE, ' ') + "\"" + getRuntimePath() + "\"\n");
        _output.push_back(Expression::createPaddedString("%include",     LABEL_TRUNC_SIZE, ' ') + "util.i\n");
        _output.push_back(Expression::createPaddedString("%include",     LABEL_TRUNC_SIZE, ' ') + "gigatron.i\n");

        if(_codeRomType >= Cpu::ROMvX0  &&  _codeRomType < Cpu::SDCARD)
        {
            _output.push_back(Expression::createPaddedString("%include", LABEL_TRUNC_SIZE, ' ') + "macros_ROMvX0.i\n");
        }
        else if(_codeRomType >= Cpu::ROMv5a)
        {
            _output.push_back(Expression::createPaddedString("%include", LABEL_TRUNC_SIZE, ' ') + "macros_ROMv5a.i\n");
        }
        else
        {
            _output.push_back(Expression::createPaddedString("%include", LABEL_TRUNC_SIZE, ' ') + "macros.i\n");
        }
        ...
        for(int i=0; i<int(_codeLines.size()); i++)
        {
            ...
            if(_codeLines[i]._code.size() >= 2  &&  _codeLines[i]._vasm.size())
            {
                ...
                for(int j=0; j<int(_codeLines[i]._vasm.size()); j++)
                {
                    ...
                    int vasmLabelIndex = _codeLines[i]._vasm[j]._labelIndex;
                    std::string vasmLabel = (vasmLabelIndex >= 0) ? _labels[vasmLabelIndex]._output : "";

                    // BASIC label has priority over vasm label
                    bool useBasicLabel = (labelIndex >=0  &&  _labels[labelIndex]._address == vasmAddress);
                    std::string label = (useBasicLabel) ? basicLabel : vasmLabel;
                    ...
                    else
                    {
                        ...
                        {
                            line += (label.size()) ?  "\n" + Expression::createPaddedString(label, LABEL_TRUNC_SIZE, ' ') + vasmCode : "\n" + std::string(LABEL_TRUNC_SIZE, ' ') + vasmCode;
                        }
                    }
                }

#define TAB_SPACE_LENGTH  4
#define COMMENT_PADDING  (TAB_SPACE_LENGTH*23)
    ...
    void clearCompiler(void)
    {
        ...
        _blitsAddrLutAddress  = 0x0000;
        _blitStripeChunks     = 15;
        _blitStripeMinAddress = USER_CODE_START;
        _blitStripeMaxAddress = RUN_TIME_START;
        ...
        _enableOptimiser = true;
        _enableRomCheck = true;
        _enableInitialise = true;
        ...
        _defDataPatterns.clear();
        _defFunctions.clear();
        _dataObjects.clear();

        _blitsAddrLut._address = 0x0000;
        _blitsAddrLut._addrs.clear();

        _fontsAddrLut._address = 0x0000;
        _fontsAddrLut._addrs.clear();

        _spritesAddrLut._addrs.clear();
        _spritesAddrLut._constantHeight = false;

        _patternAddrLuts.clear();
        _patternsAddrLut._address = 0x0000;
        _patternsAddrLut._addrs.clear();
        ...
        Memory::getFreePageRAM(Memory::FitDescending, USER_STR_SIZE + 2, USER_CODE_START, _stringsStart, _strWorkArea[0], true);
        Memory::getFreePageRAM(Memory::FitDescending, USER_STR_SIZE + 2, USER_CODE_START, _stringsStart, _strWorkArea[1], true);
    }
    ...
    bool compile(const std::string& inputFilename, const std::string& outputFilename)
    {
        ...
        Cpu::reportError(Cpu::NoError, stderr, "* Compiling file '%s'\n", inputFilename.c_str());
        Cpu::reportError(Cpu::NoError, stderr, "*******************************************************\n");

        // Default font and text handling
        pragmaPreProcessingCmd("%define FONT_BOTH", 1);
        pragmaPreProcessingCmd("%define TEXT_SCROLL", 1);
...
//#define DEBUG_DATA
        ...
        // Re-linking is needed here as collectInternalRuntime() can find new subs that need to be re-linked
