...
namespace Linker
{
    ...
    std::vector<Compiler::InternalSub> _internalSubs =
    {
        ...
        {0x0000, 0x0000, "getRomType"       , "", false, false},
        {0x0000, 0x0000, "isRomTypeX"       , "", false, false},
        ...
        {0x0000, 0x0000, "resetMem"         , "", false, false},
        ...
        {0x0000, 0x0000, "clearCursorRow4x6", "", false, false},
        ...
        {0x0000, 0x0000, "scrollBlitY"      , "", false, false},
        ...
        {0x0000, 0x0000, "mergeSpritesLut"  , "", false, false},
        ...
        {0x0000, 0x0000, "printInit4x6"     , "", false, false},
	...
	{0x0000, 0x0000, "printChar6x8"     , "", false, false},
        {0x0000, 0x0000, "printChar4x6"     , "", false, false},
        {0x0000, 0x0000, "printClip"        , "", false, false},
        {0x0000, 0x0000, "printClip4x6"     , "", false, false},
        {0x0000, 0x0000, "printHex"         , "", false, false},
        {0x0000, 0x0000, "printSpc"         , "", false, false},
        {0x0000, 0x0000, "newLineScroll"    , "", false, false},
	{0x0000, 0x0000, "newLineScroll6x8" , "", false, false},
        {0x0000, 0x0000, "newLineScroll4x6" , "", false, false},
        {0x0000, 0x0000, "atTextCursor"     , "", false, false},
        {0x0000, 0x0000, "atTextCursor4x6"  , "", false, false},
        {0x0000, 0x0000, "textWidth"        , "", false, false},
        {0x0000, 0x0000, "textWidth4x6"     , "", false, false},
        ...
        {0x0000, 0x0000, "initCursorTimer"  , "", false, false},
        {0x0000, 0x0000, "getCursorFlash"   , "", false, false},
    ...
    int getAsmOpcodeSizeOfIncludeSub(const std::string& includeName, const std::string& subName)
    {
        ...
        {
            Cpu::reportError(Cpu::LnkError, stderr, "Linker::getAsmOpcodeSizeOfIncludeSub() : Include file %s for %s was never loaded'\n", includeName.c_str(), subName.c_str());
            return -1;
        }
    ...
    bool getInternalSubCode(const std::string& includeName, const std::vector<std::string>& includeVarsDone, std::vector<std::string>& code, int subIndex)
    {
        if(_subIncludeFiles.find(includeName) == _subIncludeFiles.end())
        {
            Cpu::reportError(Cpu::LnkError, stderr, "Linker::getInternalSubCode() : Include file %s for %s was never loaded'\n", includeName.c_str(), _internalSubs[subIndex]._name.c_str());
            return false;
        }
        ...
        for(int i=0; i<int(_subIncludeFiles[includeName].size()); i++)
        {
            ...
            Expression::removeAsmComments(line);
    ...
    {
        if(_internalSubs[subIndex]._size == 0  ||  (!overwrite  &&  _internalSubs[subIndex]._address)) return false;
    ...
    bool loadInclude(const std::string& filename)
    {
        ...
        // Pre-processor, (TODO: bit'o'hack, only done for ROMvX0 FONT handling)
        // ROMvX0 FONT handling requires %if define handling of include files through pragmas
        // The current %if define system allows programmatic or pragma control, but not both simultaneously
        if(filename == "print_text_ROMvX0.i"  ||  filename == "input_ROMvX0.i")
        {
            Assembler::LineToken lToken;
            std::vector<Assembler::LineToken> lTokens;
            for(int i=0; i<int(lineTokens.size()); i++)
            {
                lToken._text = lineTokens[i];
                lTokens.push_back(lToken);
            }

            if(!Assembler::preProcess(filename, lTokens, false, false)) return false;

            lineTokens.clear();
            for(int i=0; i<int(lTokens.size()); i++)
            {
                lineTokens.push_back(lTokens[i]._text);
            }
        }
    ...
    bool linkInternalSubs(void)
    {
        ...
        for(int i=0; i<int(Compiler::getCodeLines().size()); i++)
        {
            ...
            if(Compiler::getCodeLines()[i]._code.size() >= 2  &&  Compiler::getCodeLines()[i]._vasm.size())
            {
                ...
                {
                    std::string line = Compiler::getCodeLines()[i]._vasm[j]._code;
                    Expression::removeAsmComments(line);
                    std::vector<std::string> tokens = Expression::tokenise(line, ' ');
