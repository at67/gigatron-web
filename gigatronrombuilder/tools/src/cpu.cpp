...
namespace Cpu
{
    ...
    const std::string _vCpuTraceHeader1 = "  PC  Instruction             AC    LR    SP    Fn  Arg01 Arg23 Arg45 Arg67  SrFc  0x30  0x32  0x34  0x36";
    ...
    uint64_t _vCpuNumTicks = 0;

    bool _oclk12_5 = false;
    ...
    std::map<std::string, RomType> _romTypeMap = {{"ROMV1", ROMv1}, {"ROMV2", ROMv2}, {"ROMV3", ROMv3}, {"ROMV4", ROMv4}, {"ROMV5A", ROMv5a}, {"ROMV6", ROMv6}, {"ROMVX0", ROMvX0}, {"SDCARD", SDCARD}, {"DEVROM", DEVROM}};
    std::map<RomType, std::string> _romTypeStr = {{ROMv1, "ROMv1"}, {ROMv2, "ROMv2"}, {ROMv3, "ROMv3"}, {ROMv4, "ROMv4"}, {ROMv5a, "ROMv5A"}, {ROMv6, "ROMv6"}, {ROMvX0, "ROMvX0"}, {SDCARD, "SDCARD"}, {DEVROM, "DEVROM"}};
...
//#define COLLECT_INST_STATS
    ...
    void initRomType(void)
    {
...
#if 0
        // This is the correct way to do it, but we need it's value before boot
        ...
        switch(_romType)
        {
            ...
            {
                shutdown();
                reportError(RomError, stderr, "Cpu::initRomType() : Unknown ROM Type = 0x%02x : exiting...\n", romType);
                _EXIT_(EXIT_FAILURE);
            }
            break;
        }
        //reportError(NoError, stderr, "Cpu::initRomType() : ROM Type = 0x%02x\n", _romType);
    }
    ...
    RomType readRomType(uint8_t* rom)
    {
        // This can easily fail because of random data
        if(rom[ROMADDRv1*2 + 1]  == ROMv1)  return ROMv1;
        if(rom[ROMADDRv2*2 + 1]  == ROMv2)  return ROMv2;
        if(rom[ROMADDRv3*2 + 1]  == ROMv3)  return ROMv3;
        if(rom[ROMADDRv4*2 + 1]  == ROMv4)  return ROMv4;
        if(rom[ROMADDRv5a*2 + 1] == ROMv5a) return ROMv5a;
        if(rom[ROMADDRv6*2 + 1]  == ROMv6)  return ROMv6;
        if(rom[ROMADDRvX0*2 + 1] == ROMvX0) return ROMvX0;
        if(rom[SDCARDADDR*2 + 1] == SDCARD) return SDCARD;
        if(rom[DEVROMADDR*2 + 1] == DEVROM) return DEVROM;

        return ROMERR;
    }

    void enableAudioModeRomX(AudioMode audioMode)
    {
#if 1
        switch(audioMode)
        {
        ...
        const std::vector<uint8_t>  romvXOpcodes      = {0x20,   0x00,   0x40,   0x40  };
        const std::vector<uint8_t>  romvXOperands6bit = {0x03,   0xFC,   0x03,   0x03  };
        const std::vector<uint8_t>  romvXOperands8bit = {0x00,   0xFF,   0x00,   0x00  };

        static bool firstTime = true;
        static std::map<uint16_t, uint8_t[2]> romvXBackup;

        if(firstTime) romvXBackup.clear();

        // Codes
        for(int i=0; i<int(romvXAddrs.size()); i++)
        {
            if(firstTime)
            {
                romvXBackup[romvXAddrs[i]][ROM_INST] = _ROM[romvXAddrs[i]][ROM_INST];
                romvXBackup[romvXAddrs[i]][ROM_DATA] = _ROM[romvXAddrs[i]][ROM_DATA];
            }
            ...
            switch(audioMode)
            {
                case Audio4bit:
                {
                    _ROM[romvXAddrs[i]][ROM_INST] = romvXBackup[romvXAddrs[i]][0];
                    _ROM[romvXAddrs[i]][ROM_DATA] = romvXBackup[romvXAddrs[i]][ROM_DATA];
                }
                break;

                case Audio6bit:
                {
                    _ROM[romvXAddrs[i]][ROM_INST] = romvXOpcodes[i];
                    _ROM[romvXAddrs[i]][ROM_DATA] = romvXOperands6bit[i];
                }
                break;
                ...
                {
                    _ROM[romvXAddrs[i]][ROM_INST] = romvXOpcodes[i];
                    _ROM[romvXAddrs[i]][ROM_DATA] = romvXOperands8bit[i];
                }
                break;
    ...
    void addROMS(void)
    {
        ...
        if((dir = opendir("roms")) != NULL)
        {
            while((ent = readdir(dir)) != NULL)
            {
                ...
                if(ent->d_type == DT_REG)
                {
                    ...
                    if(name.size() > ROM_EXT_SIZE  &&  name.substr(name.size() - ROM_EXT_SIZE, ROM_EXT_SIZE) == ".ROM")
                    {
        ...
        {
            std::string name = "roms/" + filenames[i];

            std::ifstream romFile(name, std::ios::binary | std::ios::in);
            if(!romFile.is_open())
            {
                reportError(RomError, stderr, "Cpu::addROMS() : failed to open ROM file : %s\n", name.c_str());
                continue;
            }

            int fileSize = 0;
            romFile.seekg(0, romFile.end);
            fileSize = int(romFile.tellg());
            ...
            if(!rom)
            {
                ...
                shutdown();
                reportError(CpuError, stderr, "Cpu::addROMS() : out of memory!\n");
                _EXIT_(EXIT_FAILURE);
            }

            romFile.read((char *)rom, sizeof(_ROM));
            if(romFile.bad()  ||  romFile.fail()  ||  fileSize != ROM_SIZE*2)
            {
                reportError(RomError, stderr, "Cpu::addROMS() : failed to read ROM file : %s\n", name.c_str());
                continue;
            }

            RomType romType = readRomType(rom);
            if(romType == ROMERR)
            {
                reportError(RomError, stderr, "Cpu::addROMS() : unknown ROM type : %s\n", name.c_str());
                continue;
            }

            _romFiles.push_back(rom);
            Editor::addRomEntry(uint8_t(romType), name);
        }
    ...
    void initialise(void)
    {
        ...
        // Last internal ROM is the one that is loaded at startup
...
#define CUSTOM_ROMV0
    ...
    void updateVcpuTrace(void)
    {
        ...
        sprintf(str, "$%04X $%04X $%04X $%04X $%04X $%04X $%04X $%04X $%04X $%04X $%04X $%04X $%04X",
                Cpu::getRAM16(0x0018), Cpu::getRAM16(0x001A),   // AC LR
                Cpu::getRAM(0x001C) | (Cpu::getRAM(0x0004)<<8), // SP
                Cpu::getRAM16(0x0022),                          // Fn
                Cpu::getRAM16(0x0024), Cpu::getRAM16(0x0026),   // Arg01 Arg23
                Cpu::getRAM16(0x0028), Cpu::getRAM16(0x002a),   // Arg45 Arg67
                Cpu::getRAM16(0x000E),                          // SrFc
                Cpu::getRAM16(0x0030), Cpu::getRAM16(0x0032),   // Vars 0x30, 0x32
                Cpu::getRAM16(0x0034), Cpu::getRAM16(0x0036));  // Vars 0x34, 0x36
    ...
    void handleMode1975(void)
    {
        ...
        {
            if(!_mode1975) _mode1975 = true;

            if(_vCpuNumTicks == MODE_1975_TICKS)
            {
                _vCpuNumTicks = 0;
                Editor::handleInput();
    ...
    bool process(bool disableInput, bool disableOutput)
    {
        ...
        // HALT vCPU, TODO: dodgey as current vCPU app may not have started at 0x0200, but will do for now
        if(disableOutput) setRAM16(0x16, 0x0200);
        ...
        {
            // Watchdog
            if(!_debugging  &&  (_clock - _vCpuStall > VCPU_STALL_CLOCKS)) vCpuStalled();
        }
        ...
        if(_vSync < 0)
        {
            ...
            if(_oclk12_5) Graphics::clearScreen12_5MHz(0);
        ...
        if(_vgaX++ < HLINE_END  &&  !disableOutput)
        {
            ...
            {
                if(_vgaX >=HPIXELS_START  &&  _vgaX < HPIXELS_END) Graphics::refreshPixel(_stateS, _vgaX-HPIXELS_START, _vgaY, _oclk12_5);
        ...
        if(_hSync > 0)
        {
            ...
            if(_vgaY >= 0  &&  _vgaY < SCREEN_HEIGHT)
            {
                ...
                if(_vgaX == 200) _oclk12_5 = false;
                if(_vgaX == 400) _oclk12_5 = true;
                ...
                if((_vgaY % 4) == 0) Graphics::refreshTimingPixel(_stateS, GIGA_WIDTH, _vgaY / 4, colour, _debugging);
            }
