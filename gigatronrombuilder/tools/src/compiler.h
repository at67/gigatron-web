...
#define OPCODE_TRUNC_SIZE  29 // The smaller you make this, the more your VASM opcode/macro names will be truncated in the resultant .vasm code
...
#define MISC_ENABLE_FNT4X6_BIT 0x10
...
#define MISC_ENABLE_FNT4X6_MSK 0xEF
...
namespace Compiler
{
    ...
    struct ElseIfData
    {
        ...
        Expression::CondCode _condCode;
        std::stack<FixUpLabel> _endIfFixUps;
    };

    struct WhileWendData
    {
        std::string _whileName;
        std::string _condName;
        std::string _condToken;
        int _codeLineIndex;
    };
    ...
    int getNextVasmLabelIndex(void);
    void setNextVasmLabelIndex(int nextVasmLabelIndex);

    void setAssignVar(bool assignVar);
    void setCodeIsAsm(bool codeIsAsm);
    void setAudioBitMask(uint8_t audioBitMask);
    ...
    int getNextFlowControlUniqueId(void);
    ...
    int findLabel(const std::string& labelName, uint16_t address = 0x0000);
    ...
    int createLabel(uint16_t address, const std::string& name, int codeLineIndex, Label& label, bool skipAddr=false, bool numeric=false, bool addUnderscore=true, bool pageJump=false, bool gosub=false, bool noPush=false);
    ...
    void createVcpuAsmLabel(int codeLineIdxBra, int vcpuAsmBra, const std::string& label);
    bool emitVcpuAsmUserVar(const std::string& opcodeStr, Expression::Numeric& numeric, bool nextTempVar);
    bool erasePragmaDefine(const std::string& define);
