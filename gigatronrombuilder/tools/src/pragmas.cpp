...
namespace Pragmas
{
    ...
    bool initialise(void)
    {
        ...
        _pragmas["_fontHandling_"]       = {"_fontHandling_",       FONTHANDLING      };
        _pragmas["_enableRomCheck_"]     = {"_enableRomCheck_",     ENABLEROMCHECK    };
        _pragmas["_enableInitialise_"]   = {"_enableInitialise_",   ENABLEINITIALISE  };
        _pragmas["_enableTextScroll_"]   = {"_enableTextScroll_",   ENABLETEXTSCROLL  };
    ...
    bool ENABLE6BITAUDIOEMU(const std::string& input, int codeLineIndex, size_t foundPos)
    {
        ...
        if(tokens[0] == "ON")
        {
            Cpu::enableAudioMode(Compiler::getCodeRomType(), Cpu::Audio6bit);
        }
        else if(tokens[0] == "OFF")
        {
            Cpu::enableAudioMode(Compiler::getCodeRomType(), Cpu::Audio4bit);
        }
        else
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::ENABLE6BITAUDIOEMU() : 'Main:%d' : syntax error, use '_enable6BitAudioEmu_ <ON/OFF>' : %s\n", codeLineIndex + 1, input.c_str());
    ...
    bool ENABLE8BITAUDIOEMU(const std::string& input, int codeLineIndex, size_t foundPos)
    {
        ...
        if(tokens[0] == "ON")
        {
            Cpu::enableAudioMode(Compiler::getCodeRomType(), Cpu::Audio8bit);
        }
        else if(tokens[0] == "OFF")
        {
            Cpu::enableAudioMode(Compiler::getCodeRomType(), Cpu::Audio4bit);
    ...
    bool FONTHANDLING(const std::string& input, int codeLineIndex, size_t foundPos)
    {
        const std::set<std::string> fontHandling = {"FONT_BOTH", "FONT_6X8", "FONT_4X6"};

        if(!Compiler::isROMvX0())
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::FONTHANDLING() : 'Main:%d' : syntax error, ROMvX required! : %s\n", codeLineIndex + 1, input.c_str());
            return false;
        }
        ...
        Expression::stripNonStringWhitespace(pragma);
        Expression::strToUpper(pragma);
        auto found = fontHandling.find(pragma);
        if(found == fontHandling.end())
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::FONTHANDLING() : 'Main:%d' : syntax error, use _fontHandling_ <FONT_BOTH/FONT_6X8/FONT_4X6> : %s\n", codeLineIndex + 1, input.c_str());
            return false;
        }

        // Erase any FONT_ defines
        Compiler::erasePragmaDefine("%define FONT_BOTH");
        Compiler::erasePragmaDefine("%define FONT_6X8");
        Compiler::erasePragmaDefine("%define FONT_4X6");
        Assembler::eraseDefine("FONT_BOTH");
        Assembler::eraseDefine("FONT_6X8");
        Assembler::eraseDefine("FONT_4X6");

        Compiler::pragmaPreProcessingCmd("%define " +  *found, codeLineIndex + 1);

        return true;
    }

    bool ENABLEROMCHECK(const std::string& input, int codeLineIndex, size_t foundPos)
    {
        UNREFERENCED_PARAM(input);
        UNREFERENCED_PARAM(codeLineIndex);
        UNREFERENCED_PARAM(foundPos);
        ...
        if(tokens.size() != 1)
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::ENABLEROMCHECK() : 'Main:%d' : syntax error, use '_enableRomCheck_ <ON/OFF>' : %s\n", codeLineIndex + 1, input.c_str());
            return false;
        }
        ...
        if(tokens[0] == "ON")
        {
            Compiler::enableRomCheck(true);
        }
        else if(tokens[0] == "OFF")
        {
            Compiler::enableRomCheck(false);
        }
        else
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::ENABLEROMCHECK() : 'Main:%d' : syntax error, use '_enableRomCheck_ <ON/OFF>' : %s\n", codeLineIndex + 1, input.c_str());
    ...
    bool ENABLEINITIALISE(const std::string& input, int codeLineIndex, size_t foundPos)
    {
        UNREFERENCED_PARAM(input);
        UNREFERENCED_PARAM(codeLineIndex);
        UNREFERENCED_PARAM(foundPos);
        ...
        if(tokens.size() != 1)
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::ENABLEINITIALISE() : 'Main:%d' : syntax error, use '_enableInitialise_ <ON/OFF>' : %s\n", codeLineIndex + 1, input.c_str());
            return false;
        }
        ...
        if(tokens[0] == "ON")
        {
            Compiler::enableInitialise(true);
        }
        else if(tokens[0] == "OFF")
        {
            Compiler::enableInitialise(false);
            Compiler::erasePragmaDefine("%define FONT_BOTH");
            Assembler::eraseDefine("FONT_BOTH");
        }
        else
    ...
    bool ENABLETEXTSCROLL(const std::string& input, int codeLineIndex, size_t foundPos)
    {
        UNREFERENCED_PARAM(input);
        UNREFERENCED_PARAM(codeLineIndex);
        UNREFERENCED_PARAM(foundPos);
        ...
        if(tokens.size() != 1)
        {
            Cpu::reportError(Cpu::PrgError, stderr, "Pragmas::ENABLETEXTSCROLL() : 'Main:%d' : syntax error, use '_enableTextScroll_ <ON/OFF>' : %s\n", codeLineIndex + 1, input.c_str());
            return false;
        }
        ...
        if(tokens[0] == "ON")
        {
            Compiler::erasePragmaDefine("%define TEXT_SCROLL");
            Assembler::eraseDefine("TEXT_SCROLL");
            Compiler::pragmaPreProcessingCmd("%define TEXT_SCROLL", codeLineIndex + 1);
        }
        else if(tokens[0] == "OFF")
        {
            Compiler::erasePragmaDefine("%define TEXT_SCROLL");
