...
namespace Assembler
{
    ...
    bool eraseDefine(const std::string& defineName)
    {
        return (_defines.erase(defineName) > 0);
    }
    ...
    bool preProcess(const std::string& filename, std::vector<LineToken>& lineTokens, bool doMacros, bool doIncludes)
    {
        Macro macro;
        ...
        for(auto itLine=lineTokens.begin(); itLine != lineTokens.end();)
        {
            ...
            // Lines containing only white space outside of SUB's are deleted
            if(!buildingSub  ||  commentPos != std::string::npos)
            {
                size_t nonWhiteSpace = lineToken._text.find_first_not_of("  \n\r\f\t\v");
                nonWhiteSpace = lineToken._text.find_first_not_of("  \n\r\f\t\v");
                if(nonWhiteSpace == std::string::npos)
                {
            ...
            if(tokens.size() > 0)
            {
                ...
                // Whitespace outside of SUB's is deleted
                if(tokens[0] == "%SUB")  buildingSub = true;
                if(tokens[0] == "%ENDS") buildingSub = false;

                // Remove subroutine header and footer
                if(doIncludes  &&  (tokens[0] == "%SUB"  ||  tokens[0] == "%ENDS"))
                {
                    itLine = lineTokens.erase(itLine);
                    continue;
                }
                ...
                // Include
                if(doIncludes  &&  tokens[0] == "%INCLUDE")
                {
                ...
                // Include path
                else if(doIncludes  &&  tokens[0] == "%INCLUDEPATH"  &&  tokens.size() > 1)
                {
            ...
            // Check if there is a current define and delete code line if define is disabled
            if(!_currentDefine.empty())
            {
                std::string define = _currentDefine.top();
                if(_defines.find(define) == _defines.end())
                ...
                // Inherit parent enables
                bool nestedEnable = true;
                std::stack<std::string> currentDefines = _currentDefine;
                while(!currentDefines.empty())
                {
                    nestedEnable &= _defines[currentDefines.top()]._enabled;
                    currentDefines.pop();
                }
        ...
        if(!_currentDefine.empty())
        {
            Cpu::reportError(Cpu::AsmError, stderr, "Assembler::preProcess() : '%s:%d' : missing '%%endif' for %s\n", filename.c_str(), adjustedLineIndex, _currentDefine.top().c_str());
            return false;
        }
