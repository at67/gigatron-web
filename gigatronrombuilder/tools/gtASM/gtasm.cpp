...
int main(int argc, char* argv[])
{
    if(argc != 2  &&  argc != 3)
    {
        Cpu::reportError(Cpu::UtlError, stderr, "%s\n", GTASM_VERSION_STR);
        Cpu::reportError(Cpu::UtlError, stderr, "Usage:   gtasm <input filename> <optional include path>\n");
        return 3;
    }
    ...
    {
        Cpu::reportError(Cpu::UtlError, stderr, "Wrong file extension in %s : must be one of : '.vasm' or '.gasm' or '.asm' or '.s'\n", name.c_str());
        return 3;
    }
    ...
    address = Assembler::getStartAddress();
    ...
    if(!hasRomCode  &&  !saveGt1File(filename, gt1File, gt1FileName)) return 2;
